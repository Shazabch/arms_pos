{*
12/12/2013 4:46 PM Andy
- Allow category autocomplete to accept parameter "min_level" and "skip_dept_filter".
*}
...
CAT_AUTOCOMPLETE_MAIN{$ext} {literal} = {
...
<table width="100%">
    ...
    <tr>
        ...
        <td>
        	<input type="hidden" id="inp_category_id{$ext}" name="category_id" >
            <input type="text" id="inp_search_cat_autocomplete{$ext}" name="search_cat_autocomplete" size="30" style="font-size:14px;width:400px;" onClick="this.select();" />
            <input type="button" id="inp_add_cat_autocomplete{$ext}" value="{$add_value|default:'Add'}" />
            <div id="div_search_cat_autocomplete_choices{$ext}" class="autocomplete" style="display:none;height:150px !important;width:400px !important;overflow:auto !important;z-index:100"></div>
        </td>
    </tr>
    <tr>
    	<td>&nbsp;</td>
    	<td>
    		<div id="div_category_autocomplete_description{$ext}" style="color:blue;">
    		</div>
    	</td>
    </tr>
</table>
<div style="height:20px;margin-top:-20px;">
	<span id="span_category_autocomplete_loading{$ext}" style="padding:2px;background:yellow;display:none;"><img src="ui/clock.gif" align="absmiddle" /> Loading...</span>
</div>