...
<p style="color:blue;">
	...
	* Please ENSURE the result data is fill to the header accordingly before proceed to import.<br />
	<br/>
	<input type="button" id="import_btn" name="import_btn" value="Import" onclick="GRN_UPLOAD_CSV.import_grn({$method});" {if !$result.import_row}disabled{/if} />
...
<div class="div_tbl">
	<table id="si_tbl" width="100%">
		<tr bgcolor="#ffffff">
			<th><input type="checkbox" onclick="GRN_UPLOAD_CSV.check_all_item(this);" /></th>
			<th>#</th>
		...
		<tbody>
		{foreach from=$item_lists item=i name=grn}
			<tr class="{if $i.error}tr_error{/if}">
				<td align="center"><input id="grn_items_list-{$smarty.foreach.grn.iteration}" type="checkbox" name="grn_tmp_item[]" value="{$smarty.foreach.grn.iteration}" {if $i.error}disabled{else}checked{/if} /></td>
				<td align="center">{$smarty.foreach.grn.iteration}.</td>
