...
<div class="card mx-3">
	<div class="card-body">
		...
		<div id="div_qty_implode_explode" style="{if $form.bom_type eq 'package'}display:none;{/if}">
			<b class="form-label">Qty :</b> <input name=qty_bom size=8 class="r" onchange="mi(this);"> &nbsp;&nbsp;
			<input class="btn btn-success" type=button value="Implode" onclick="do_implode();"> &nbsp;&nbsp;
			<input class="btn btn-danger" type=button value="Explode" onclick="do_explode();">
			<br><br>
		</div>
		{/if}
		...
		<tr>
		<td><b class="form-label">Product Description</b></td>
		<td>
		<input class="form-control" id="description" name="description" size="80" value="{$form.description|escape:html}" onchange="uc(this);{if $config.masterfile_disallow_double_quote}check_description(this);{/if} add_to_sku_receipt_desc('description', 'receipt_description');"> <img src="ui/rq.gif" align="absbottom" title="Required Field">
		</td>
		</tr>
		
		<tr>
		<td><b class="form-label">Receipt Description [<a href="javascript:void(alert('Max 40 characters for alphabetical character (Example: English) \nMax 13 characters for non alphabetical character (Example: Chinese)'))">?</a>]</b></td>
		<td>
		<input class="form-control" id="receipt_description" name="receipt_description" size="50" value="{$form.receipt_description|escape:html}" onchange="uc(this);{if $config.masterfile_disallow_double_quote}check_description(this);{/if} update_sku_receipt_desc(this);" maxlength="30"> <img src="ui/rq.gif" align="absbottom" title="Required Field">
		</td>
		</tr>
		
		<tr>
		<td><b class="form-label">Article No.</b></td>
		<td>
		<input class="form-control" name=artno value="{$form.artno}" {if $form.disabled_edit}readonly{/if} maxlength="30" onchange="check_artmcode(this,'artno')">
		</td>
		</tr>
		
		{if $config.sku_bom_show_mcode}
		<tr>
		<td><b class="form-label">Manufacturer's Code</b></td>
		<td>
		<input class="form-control" name=mcode value="{$form.mcode}" {if $form.disabled_edit}readonly{/if} maxlength="30" onchange="check_artmcode(this,'mcode')">
		</td>
		</tr>
		{/if}
		...
		{if $config.sku_bom_additional_type}
			<tr>
				<td><b class="form-label">Type [<a href="javascript:void(alert('Package: sell as BOM content in POS'))">?</a>]</b></td>
				<td>
					{if $form.disabled_edit}
						{if $form.bom_type eq 'normal'}Normal{else}Package{/if}
					{/if}
					<select class="form-control" name="bom_type" {if $form.disabled_edit}style="display:none;"{/if} onChange="BOM_EDITOR.bom_type_changed();" >
		...
		{else}
			<tr>
				<td><b class="form-label">Type</b></td>
				<td>
		...
		<tr>
			...
			<td>
				<select class="form-control" name="output_tax" onchange="BOM_EDITOR.calc_gst();">
		...
		<tr>
			...
			<td>
				<select class="form-control" name="inclusive_tax" onchange="BOM_EDITOR.calc_gst();">
		...
		<tr>
			<td valign="top"><b class="form-label">Selling Price Settings</b></td>
			<td>
		...
		<tr>
		<td><b class="form-label">Selling Price<span class="text-danger" title="Required Field"> *</span></b></td>
		<td>
			...
			<tr>
				<td>
					...
					&nbsp;&nbsp;<input class="form-control" name="selling_price" size=8 onchange="mf(this); BOM_EDITOR.calc_gst();" value="{$form.selling_price|default:0|number_format:2}" {if $form.disabled_edit || $form.bom_type eq 'package'}readonly{/if} onclick="this.select();"> 
		...
		<tr>
		<td><b class="form-label">Misc Cost</b></td>
		<td>
			<div class="form-inline">
				{$config.arms_currency.symbol}
		&nbsp;&nbsp;<input class="form-control" size=8 id=misc_cost name=misc_cost value="{$form.misc_cost+0|number_format:2}" onchange="mf(this);calc_all();" onclick="this.select();" {if $form.disabled_edit}readonly{/if}>
			</div>
		</td>
		</tr>
		
		<tr>
		<td><b class="form-label">Cost Price<span class="text-danger" title="Required Field"> *</span></b></td>
		<td>
			<div class="form-inline">
				{$config.arms_currency.symbol}&nbsp;&nbsp;
		<input class="form-control" id=cost_price name=cost_price size=8 onchange="float(round(this,global_cost_decimal_points));" value="{$form.cost_price+0|number_format:$config.global_cost_decimal_points}" readonly {if !$sessioninfo.privilege.SHOW_COST}style="display:none;"{/if}>
		{if $sessioninfo.privilege.SHOW_COST}<span>{/if}
		...
		<tr>
			<td>
			<b class="form-label">Item Location</b></td>
			<td>
			<input class="form-control" onchange="ucz(this)" name="location" size=6 value="{$form.location}">
			</td>
		</tr>
		
		<tr>
			<td><b class="form-label">Block item in PO</b></td>
		...
		<tr>
			<td><b class="form-label">Active</b></td>
			<td>
		...
		<!-- Category Discount -->
		<tr>
			<td valign="top"><b class="form-label">Category Discount (%)</b>
		...
		<!-- Reward Point -->
		<tr>
			<td valign="top"><b class="form-label">Reward Point</b>
		...
		{if $config.sku_non_returnable}
			<tr valign="top">
				<td nowrap><b class="form-label">Non-returnable</b> <a href="javascript:void(alert('Turn on this will not allow this SKU to return at GRA'))"><img src="/ui/icons/information.png" align="absmiddle" /></a></td>
				<td>
					<select class="form-control" name="non_returnable">
		...
		<tr>
			<td>
			<b class="form-label">Weight in KG</b></td>
			<td>
			<input class="form-control" name="weight_kg" size=6 value="{$form.weight_kg|ifzero:''}">
			</td>
		</tr>
		
		<tr>
			<td>
			<b class="form-label">Width</b></td>
			<td>
			<div class="form-inline">
				<input class="form-control" name="width" size=6 value="{$form.width|ifzero:''}">&nbsp; cm
			</div>
			</td>
		</tr>
		...
		<tr>
			<td>
			<b class="form-label">Height</b></td>
			<td>
				<div class="form-inline">
					<input class="form-control" name="height" size=6 value="{$form.height|ifzero:''}">&nbsp; cm
				</div>
			</td>
		</tr>
		...
		<tr>
			<td>
			<b class="form-label">Length</b></td>
			<td>
			<div class="form-inline">
				<input class="form-control" name="length" size=6 value="{$form.length|ifzero:''}">&nbsp; cm
			</div>
			</td>
		</tr>
		...
		<tr>
			<td>
			<b class="form-label">Marketplace<br>Description</b></td>
			<td>
			<textarea class="form-control" id="" cols="30" rows="5" name="marketplace_description">{$form.marketplace_description|escape}</textarea>
			</td>
		</tr>
		{/if}
	...
	<div >
		<div >
			<div class="table-responsive">
				<table width=100% id=tbl_item style=" padding:5px; background-color:#fe9"  class="input_no_border body table mb-0 text-md-nowrap  table-hover" cellspacing=1 cellpadding=1>
					...
					<tbody class="fs-08" id="docs_items">
		...
		{if !$form.disabled_edit}
		{*
		<table id=tbl_sku width=100% style="border:1px solid #999; padding:2px; >
		<tr class=normal>
		<td nowrap>
