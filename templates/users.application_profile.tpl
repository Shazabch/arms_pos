...
<h1>User EForm Application Profile</h1>
<div class=errmsg>
{if $errmsg.a}
<ul>
	...
	<form name=f_a method=post >
		...
		<div style="width: 50%">
			<table cellspacing="4" cellpadding="3"  {if $user.status eq 0}width="85%"{else}width="58%"{/if}>
				...
				<tr>
					<td><b>Remarks</b></td>
					<td><textarea name="remarks" id="" cols="28" rows="3"></textarea></td>
				</tr>	
				{else}
					<tr>
						<td><b>Template</b></td>
		...
		<p align=center>
			{if !$approved_by and $sessioninfo.privilege.USERS_EFORM_APPROVAL and $can_approve_user}
				...
				<input class="btn btn-error" name=reject onclick="user_action(2)" type=button value="Reject">
			{else}
				<input class="btn" type="button" onclick="document.location='users.application.php?a=application_list'" value="Close">
			{/if}
		</p>
	</form>
</div>
