...
<tr>
<th align=left>Acct Code {$n+1}</th>
{section name=b loop=$branches}
{assign var=bid value=$branches[b].id}
<td>
<input id=acct_code_{$bid} name="acct_code[{$bid}][]" size=10 maxlength=8 value="{$vvc.acct_code.$bid.$n}" onchange="uc(this);" {if !$sessioninfo.privilege.MST_VENDOR}disabled{/if}>
</td>
{/section}
</tr>
{/section}
