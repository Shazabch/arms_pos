	...
	<div class="card-body">
		<table  border=0 cellpadding=2 cellspacing=1>
			<tr>
					...
					<input class="input_artno form-control" item_id="{$item_n}" name="artno[{$item_n|default:0}]"
					...
					{if !$config.sku_application_allow_no_artno_mcode}{/if}
					{if !$config.masterfile_disable_auto_explode_artno}
						&nbsp;&nbsp;&nbsp;&nbsp;
						<b class="form-label mt-2">Article Size</b>&nbsp;&nbsp;
						<input class="form-control" onchange="this.value=this.value.trim().toUpperCase();check_artmcode(this,'artno','{$item_n}')" name="artsize[{$item_n|default:0}]" value="{$items[$item_n].artsize}">
					{/if}
			
			</tr>
			<tr>
				<b class="form-label mt-2">Manufacturer's Code <span class="text-danger"> *</span></b>
				<input class="form-control" onchange="check_artmcode(this,'mcode','{$item_n}')" name="mcode[{$item_n|default:0}]" value="{$items[$item_n].mcode}" maxlength="15"> {if !$config.sku_application_allow_no_artno_mcode}{/if}
			</tr>
			
			{if $config.sku_application_show_linkcode}
				<tr>
					<b class="form-label mt-2">{$config.link_code_name}</b>
					
						<input class="form-control" name="link_code[{$item_n|default:0}]" value="{$items[$item_n].link_code}" maxlength="20" />
			...
			<tr>
				<b class="form-label mt-2">Packing UOM</b>
				<select class="form-control" name="packing_uom_id[{$item_n}]" onchange="calc_weight_kg({$item_n});">
			...
			<tr>
				<b class="form-label mt-2">Weight in KG <a href="javascript:void(alert('Weight in KG is use for Work Order module and Self Checkout Counter from v199 and above.'))"><img src="/ui/icons/information.png" align="absmiddle" /></a></b>
					<input class="form-control"  type="text" name="weight_kg[{$item_n|default:0}]" value="{$items[$item_n].weight_kg|weight_nf}" size="10" maxlength="15" {if $item_n ne 0}readonly{else}onchange="mf(this, {$config.global_weight_decimal_points}); calc_weight_kg();"{/if} />
			...
			<tr>
				...
				<input class="form-control" name="description0[{$item_n|default:0}]" value="{$items[$item_n].description0|escape}" type=hidden>
				(Weight Description) <a href="javascript:void(alert('This weight settings is used for display purpose only.'))"><img src="/ui/icons/information.png" align="absmiddle" /></a>&nbsp;
				<input class="form-control" onchange="uc(this);atom_update_fulldesc({$item_n|default:0})" size=10 maxlength=15  name="description1[{$item_n|default:0}]" value="{$items[$item_n].description1|escape}">
				<div class="row mt-2">
					<div class="col-md-6">
						<span class="mt-2">(Sz)</span> <input class="form-control" id="autocomplete_size_{$item_n}" onblur="uc(this);atom_update_fulldesc({$item_n|default:0})" onkeydown="autocomplete_color_size_variety('size',{$item_n})" size=10 maxlength=15 name="description2[{$item_n|default:0}]" value="{$items[$item_n].description2|escape}" autocomplete="off">
					<div id="div_autocomplete_size_choices_{$item_n}" class="autocomplete" style="display:none;"></div>
					</div>
					<div class="col-md-6">
						<span class="mt-2">(Colour)</span><input class="form-control" id="autocomplete_color_{$item_n}" onblur="uc(this);atom_update_fulldesc({$item_n|default:0});" onkeydown="autocomplete_color_size_variety('color',{$item_n});" size=10 maxlength=15 name="description3[{$item_n|default:0}]" value="{$items[$item_n].description3|escape}" autocomplete="off">
					<div id="div_autocomplete_color_choices_{$item_n}" class="autocomplete" style="display:none;" ></div>
					</div>
					<div class="col-md-6">
						<span class="mt-2">(Flavour)</span><input class="form-control" onchange="uc(this);atom_update_fulldesc({$item_n|default:0})" size=10 maxlength=15 name="description4[{$item_n|default:0}]" value="{$items[$item_n].description4|escape}">
					</div>
					<div class="col-md-6">
						<span class="mt-2">(Misc)</span> <input class="form-control" onchange="uc(this);atom_update_fulldesc({$item_n|default:0})" size=20 maxlength=30 name="description5[{$item_n|default:0}]" value="{$items[$item_n].description5|escape}">
					</div>
				</div>
				
			</tr>
			<tr>
				<b class="form-label mt-2">Product Description <span class="text-danger"> *</span></b>
				<input class="form-control" size=80 id="description-{$item_n|default:0}" name="description[{$item_n|default:0}]" onchange="uc(this); {if $config.masterfile_disallow_double_quote}check_description(this);{/if} add_to_sku_receipt_desc('description-{$item_n|default:0}','receipt_description-{$item_n|default:0}');" style="background-color:#fff" value="{$items[$item_n].description|escape}">
				 
				<div class="small text-danger" >Eg: [brand] [flavour] [product name] [weight/size/colour] [misc]</div>
				
			</tr>
			
			
			<tr>
				<b class="form-label mt-2">Receipt Description [<a href="javascript:void(alert('Max 40 characters for alphabetical character (Example: English) \nMax 13 characters for non alphabetical character (Example: Chinese)'))">?</a>]</b>
				
				<input class="form-control" onblur="uc(this)" {if $config.masterfile_disallow_double_quote}onchange="check_description(this); update_sku_receipt_desc(this);"{else}onchange="update_sku_receipt_desc(this)"{/if} onchange="check_receipt(this,'receipt_description'，'$item_n'" size="50" maxlength="40" id="receipt_description-{$item_n|default:0}" name="receipt_description[{$item_n|default:0}]" value="{$items[$item_n].receipt_description|escape:"html"}"  class="inp_receipt_desc" /> 
				<input class="receipt_desc form-control" type="hidden" value="receipt_description-{$item_n|default:0}" />
			...
			{if $config.sku_enable_additional_description}
				<tr>
					<b class="form-control">Additional Description</b>
					
						<input class="form-control" type="checkbox" name="additional_description_print_at_counter[{$item_n|default:0}]" value="1" {if $items[$item_n].additional_description_print_at_counter}checked{/if} /> Print at Counter
						&nbsp;&nbsp;&nbsp;
						<input class="form-control" type="checkbox" name="additional_description_prompt_at_counter[{$item_n|default:0}]" value="1" {if $items[i].additional_description_prompt_at_counter}checked{/if} /> Prompt when scan at POS Counter
						<br />
						<textarea class="form-control" cols="45" rows="6" onblur="uc(this)" {if $config.masterfile_disallow_double_quote}onchange="check_description(this);"{/if} name="additional_description[{$item_n|default:0}]">{$items[$item_n].additional_description|escape}</textarea>
			...
			<tr class="gst_settings">
				<b class="form-label mt-2">Input Tax</b>
					<select class="form-control select2" name="dtl_input_tax[{$item_n}]" class="dtl_input_tax">
			...
			<tr class="gst_settings">
				<b class="form-label mt-2">Output Tax</b>
					<select class="form-control select2" name="dtl_output_tax[{$item_n}]" onchange="calculate_gst({$item_n}, this);" class="dtl_output_tax">
			...
			<tr style="{if !$gst_settings or ($global_gst_settings.inclusive_tax eq 'yes' and (!isset($items.$item_n.inclusive_tax) or $items.$item_n.inclusive_tax eq 'inherit'))}display:none;{/if}">
				<b class="form-label mt-2">Selling Price Inclusive Tax</b>
					<select class="form-control select2" name="dtl_inclusive_tax[{$item_n}]" onchange="calculate_gst({$item_n}, this);" class="dtl_inclusive_tax">
		...
		<div class="row">
			<div class="col-md-6">
				<tr>
					<b class="form-label mt-2">Selling Price Settings</b>
			...
			{* Use RSP *}
			<tr>
				<span class="form-inline form-label mt-2">
					<b>RSP</b> <a href="javascript:void(prompt_rsp_notification())">[?]</a>
				</span>
					...
					<table>
						<tr>
							<span class="form-inline"><b class="form-label mt-2">RSP</b>
								&nbsp;<span class="fs-07">{$config.arms_currency.symbol}</span>
								&nbsp;&nbsp;&nbsp;<input class="form-control" type="text" name="rsp_price[{$item_n|default:0}]" value="{$items[$item_n].rsp_price|number_format:2:".":""}" size="6" {if !$items[$item_n].use_rsp}readonly {/if} onChange="rsp_price_changed('{$item_n}');" />
							
						</tr>
						<tr>
								<span class="form-inline form-label mt-2"><b>RSP Discount <a href="javascript:void(show_discount_help())">[?]</a></b>
							
								&nbsp;&nbsp;&nbsp;<input class="form-control" type="text" name="rsp_discount[{$item_n|default:0}]" value="{$items[$item_n].rsp_discount}" {if !$items[$item_n].use_rsp}readonly {/if} onChange="rsp_discount_changed('{$item_n}');" />
			...
			<tr>
				<b class="form-label">Selling Price <span class="text-danger"> *</span></b>
					<table >
						<tr >
								{$config.arms_currency.symbol} <input class="form-control" name="selling_price[{$item_n|default:0}]" size=6 value="{$items[$item_n].selling_price|number_format:2:".":""}" onchange=" item_selling_price_changed('{$item_n}');"> 
				...
				<td class="gst_settings" nowrap><b class="form-label mt-2">Selling Price <span id="span_gst_indicator_{$item_n}"></span> GST</b></td>
				<td class="gst_settings">{$config.arms_currency.symbol} 
				<input class="form-control" type="text" name="gst_selling_price[{$item_n|default:0}]" value="" size="6" onchange="this.value=round2(this.value); calculate_gst({$item_n}, this);" /></td>
			</tr>
			
			{if $config.do_enable_hq_selling and $BRANCH_CODE eq 'HQ'}
			<tr>
				<b class="for-label mt-2">HQ Selling</b>
				{$config.arms_currency.symbol} 
				<input class="form-control" name="hq_selling[{$item_id}]" size=6 value="{$items[$item_n].hq_selling|number_format:2:".":""}" onchange="this.value=round2(this.value);"></td>
			</tr>
			{/if}
			<tr>
				<b class="form-label mt-2">Cost Price <span class="text-danger"> *</span></b>
				(Enter {$config.arms_currency.symbol} or %) 
				<input class="form-control" name="cost_price[{$item_n|default:0}]" size=6 value="{$items[$item_n].cost_price|number_format:4:".":""}" onchange="cost_changed(this, '{$item_n}');"> </td>
			</tr>
			{if $config.sku_listing_show_hq_cost and $BRANCH_CODE eq 'HQ'}
			<tr>
				<b class="form-label mt-2">HQ Cost</b>
				{$config.arms_currency.symbol}
				 <input class="form-control" name="hq_cost[{$item_id}]" size=6 value="{$items[$item_n].hq_cost|number_format:2:".":""}" onchange="this.value=round2(this.value);"></td>
			</tr>
			{/if}
			<tr>
				<b class="form-label mt-2">Gross Profit</b>
				<div class="row">
					<div class="col-md-6">
						<td>{$config.arms_currency.symbol} <input class="form-control" name="gross[{$item_n|default:0}]" size=6 readonly></td>
						
					</div>
					<div class="col-md-6">
						<span class="form-inline"><td><b >GP(%) [<a href="javascript:void(alert('{$LANG.SKU_GP_PER_LEGEND}'))">?</a>]</b></td></span>
						<td><input class="form-control" name="grossp[{$item_n|default:0}]" size=6 readonly> </td>
					</div>
				</div>
			</tr>
			{if $config.masterfile_sku_enable_ctn}
				<div class="row">
					<div class="col-md-6">
						<tr>
							<b class="form-label mt-2">Ctn #1</b>
								<select class="form-control select2" name="ctn_1_uom_id[{$item_n|default:0}]">
					...
					<div class="col-md-6">
						<tr>
							<b class="form-label mt-2">Ctn #2</b>
								<select class="form-control mt-2" name="ctn_2_uom_id[{$item_n|default:0}]">
									{foreach from=$uom item=u}
			...
			<tr>
				<span class="from-inline form-label mt-2"><b class="">Allow Decimal Qty</b> [<a href="javascript:void(alert('{$LANG.SKU_ALLOW_DECIMAL_NOTIFICATION|escape:javascript}'));">?</a>]</td></span>
			...
			{if $config.enable_replacement_items}
				<tr>
					|<b class="form-label mt-2">Replacement Item Group</b>
						<input  type="checkbox" onClick="toggle_replacement_group(this, '{$item_n|default:0}');" {if $items.$item_n.ri_id}checked {/if} />
						<input class="form-control" type="text" readonly size="1" name="ri_id[{$item_n|default:0}]" id="inp_ri_id_{$item_n|default:0}" value="{$items.$item_n.ri_id}" {if !$items.$item_n.ri_id}disabled {/if} />
						<input class="form-control" type="text" name="ri_group_name[{$item_n|default:0}]" size="40" id="inp_ri_group_name_{$item_n|default:0}"  value="{$items.$item_n.ri_group_name}"  {if !$items.$item_n.ri_id}disabled {/if} />
		...
		<div class="row">
			<div class="col-md-6">
				<tr>
					<b class="form-label mt-2">Category Discount (%)</b>
			...
			<div class="col-md-6">
				<tr>
					<b class="form-label mt-2">Reward Point</b>
			...
			{if !$config.consignment_modules}
				<tr>
					<b class="form-label mt-2">Scale Type</b>
						<select class="form-control select2" name="dtl_scale_type[{$item_n|default:0}]">
			...
			{if $config.sku_non_returnable}
				<tr valign="top">
					<b class="form-label mt-2">Non-returnable</b> <a href="javascript:void(alert('Turn on this will not allow this SKU to return at GRA'))"><img src="/ui/icons/information.png" align="absmiddle" /></a></td>
					
						<select class="form-control" name="non_returnable[{$item_n}]">
			...
			<tr>
					<b class="form-label mt-2">Model</b>
					<input class="form-control" type="text" name="model[{$item_n}]" value="{$items[$item_n].model}" />
			...
			<div class="col-md-4">
				<tr class="col-md-4">
					<b class="form-label mt-2">Width</b>
					<input class="form-control" type="text" name="width[{$item_n}]" value="{$items[$item_n].width}" onChange="mfz(this, 2)" cm
				
			</tr>
			</div>
			<div class="col-md-4">
			<tr class="col-md-4">
				<b class="form-label mt-2">Height</b>
					<input class="form-control" type="text" name="height[{$item_n}]" value="{$items[$item_n].height}" onChange="mfz(this, 2)" cm
				</td>
			</tr>
		</div>
		<div class="col-md-4">
			<tr class="col-md-4">
				<b class="form-label mt-2">Length</b>
					<input class="form-control" type="text" name="length[{$item_n}]" value="{$items[$item_n].length}" onChange="mfz(this, 2)" cm
				</td>
			</tr>
			</div>
			...
			{if $sku_extra_info}
				<tr>
					<td colspan="2">
						<fieldset>
							<legend><b class="form-label">Extra Info</b></legend>
							<table>
								{foreach from=$sku_extra_info key=c item=extra_info}
									<tr>
										<b>{$extra_info.description}</b>
											{if $extra_info.input_type eq 'text'}
												<input class="from-control" type="text" name="extra_info[{$item_n}][{$c}]" value="{$items.$item_n.extra_info.$c}" {if $extra_info.data_type eq 'date'}onchange="date_validate(this);"{/if} />
			...
			<tr class="tr_si_po_reorder_qty">
				<td><b class="form-label mt-2">PO Reorder Qty</b></td>
				<td>
					<div class="row ">
						<div class="col-md-4">
							Min: <input type="text" class="form-control" size="3" name="si_po_reorder_qty_min[{$item_n|default:0}]" value="{$items.$item_n.po_reorder_qty_min}" class="si_po_reorder_qty" />
						</div>
						<div class="col-md-4">
							Max: <input type="text" class="form-control" size="3" name="si_po_reorder_qty_max[{$item_n|default:0}]" value="{$items.$item_n.po_reorder_qty_max}" class="si_po_reorder_qty" />
						</div>
						<div class="col-md-4">
							MOQ: <a href="javascript:void(alert('Minimum Order Quantity'))"><img src="/ui/icons/information.png" align="absmiddle" /></a> 
							<input class="form-control" type="text" size="3" name="si_po_reorder_moq[{$item_n|default:0}]" value="{$items.$item_n.po_reorder_moq}" class="si_po_reorder_qty" />
						</div>
				
					</div>
					
					<b class="form-label ">Notify Person </b>
					<select class="form-control select2" name="si_po_reorder_notify_user_id[{$item_n|default:0}]" class="si_po_reorder_qty">
			...
			{if $config.enable_sn_bn}
				<tr>
					<b class="form-label">Warranty Period</b>
						<input class="form-control" name="sn_we[{$item_n}]" size="6" value="{$items[$item_n].sn_we}" class="r" onchange="mi(this);">&nbsp;
						<select class="form-control select2" name="sn_we_type[{$item_n}]">
			...
			{if $sessioninfo.privilege.MST_INTERNAL_DESCRIPTION}
				<tr>
						<b class="form-label mt-2">Internal Description</b>
						<textarea class="form-control" cols="45" rows="6" onblur="uc(this)" name="internal_description[{$item_n}]">{$items.$item_n.internal_description|escape}</textarea>
			...
			<tr>
				<b class="form-label mt-2">Marketplace Description</b>
					<textarea class="form-control"   cols="45" rows="6" onblur="uc(this)" name="marketplace_description[{$item_n}]">{$items.$item_n.marketplace_description|escape}</textarea>
				</td>
			</tr>
			{/if}
