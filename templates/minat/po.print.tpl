...
1/22/2013 4:29 PM Justin
- Enhanced to add "Approved by" signature area.
- Enhanced to add new note into IMPORTANT area.
- Enhanced to adjust the Approved By to fill into one line.
...
<tr>
	...
	<h2>{$billto.description}</h2>
		...
		&nbsp;&nbsp; Fax: {$billto.phone_3}
		{if $config.enable_gst and $form.is_under_gst}
			&nbsp;&nbsp;&nbsp;&nbsp;GST No: {$billto.gst_register_no}
		{/if}
	</td>
...
iv) Prices quality of merchandise if differ from original sample will be liable for deduction or return.<br>
v) Outright purchase is under returnable basic.
</td>
...
{else}
<tr>
<td width="21%" valign=bottom class=small>
...
<td width="35%" valign=top class=xsmall>
<b>IMPORTANT:</b><br>
i) Please quote our P/O No. in all your Delivery Order / Invoices.<br>
ii) Kindly supply goods in strict accordance to our P/O.<br>
iii) Valid only if goods according to Trade Description.<br>
...
<td width="23%" valign=top class=xsmall>
<b>NOTICE:</b><br>
NOTICE is hereby given that the undersigned Company will strictly abandond and reject any Supplier who has factoring arrangement with financial institution.
</td>
</tr>
...
<tr height=15 class="rw{cycle name=crow values=",2"} no_border_bottom">
	...
	{if !$page_item_info.$item_id.not_item}
		...
		<td {if !$form.is_under_gst || !$sp_inclusive_tax}rowspan=2{/if} nowrap align=right style="border-top:1px solid #000;{if $sell<$cost}font-weight:bold;{/if}">
			{if $form.is_under_gst && $item.inclusive_tax eq 'yes'}
				{$item.gst_selling_price|number_format:3}
			{else}
...
<tr height=30>
	...
	{if $form.is_under_gst}
		{*assign var=total_profit value=$form.total_selling_amt-$total.gst_amount*}
		<td class="ft" nowrap align=right>{$form.subtotal_po_gst_amount|number_format:2}</td>
		<td class="ft" nowrap align=right>{$form.subtotal_po_amount_incl_gst|number_format:2}</td>
	{/if}
</tr>
