...
<tr bgcolor=#cccccc class="small">
	...
	{if !$form.show_per && $form.is_under_gst}
		<th width=80>GST<br>({$config.arms_currency.symbol})</th>
		<th width=80>Amount <br>Incl. GST ({$config.arms_currency.symbol})</th>
	{/if}
	{if $form.show_per}
		<th width="30">Disc (%)</th>
		<th width="80">Net ({$config.arms_currency.symbol})</th>
	...
	{if $to_branch.currency_code && $to_branch.currency_code ne $config.arms_currency.symbol}
		<th width="80">Net<br />({$to_branch.currency_code})</th>
	{/if}
</tr>
...
<tr class="no_border_bottom {if $smarty.section.i.iteration eq $PAGE_SIZE and !$is_lastpage}td_btm_got_line{/if}">
	...
	{if $form.show_per}
		...
		<td align="right">{$ci_items[i].item_amt|number_format:2}</td>
		{if $form.is_under_gst}
			<td align="right">{$ci_items[i].item_gst|number_format:2}</td>
			<td align="right">{$ci_items[i].item_gst_amt|number_format:2}</td>
		{/if}
	...
	{assign var=total2_row value=$total2_row|round:3}
	{assign var=total2 value=$total2+$total2_row}
...
<tr class="total_row">
	...
	<td rowspan="3" colspan="{$cols}">Remark<br />{$form.remark|default:"-"|nl2br}
	{if $form.discount_selling_price_percent}
		<span class="small">
		<br />Discount Selling Price (%): {$form.discount_selling_price_percent}&nbsp;&nbsp;&nbsp;&nbsp;
		</span>
	{/if}
	{if $form.discount_item_row_percent}
		<span class="small">
		<br />Bearing Percent(%): {$form.discount_item_row_percent}&nbsp;&nbsp;&nbsp;&nbsp;
		</span>
	{/if}
	</td>
