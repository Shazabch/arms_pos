...
7/7/2010 11:27:43 AM Andy
- Show department for DO/Invoice/pre-checkout printing. (if got config and department)
- DO invoice remove last page signature.
...
7/15/2011 11:24:57 AM Andy
- Add include 'header.print.tpl' for all printing templates, added support charset utf8.

8/9/2011 3:31:20 PM Andy
...
10/7/2011 11:20:42 AM Alex
- add $config.global_cost_decimal_points & qty_nf control cost and qty decimal point

8/14/2012 10:44 AM Andy
- Add  Issued By,  Approved By and  Received By signature.
...
3/23/2015 2:51 PM Andy
- Enhance custom template to compatible with GST.

4/20/2015 5:40 PM Andy
- Change the word "Invoice" to "Tax Invoice".
...
4/21/2017 1:45 PM Khausalya 
- Enhanced changes from RM to use config setting. 

7/20/2017 14:18 Qiu Ying
...
<tr>
	...
	<h2>{$from_branch.description}</h2>
		...
		{* if $config.enable_gst and $from_branch.gst_register_no}
			 GST No: {$from_branch.gst_register_no}<br />
		{/if *}
...
{if $form.is_under_gst}TAX {/if}INVOICE</b></div>
		{if $copy_type ne 'normal'}<div style="text-align:right;">{$copy_type}</div>{else}<br>{/if}
		</td></tr>
		<tr bgcolor="#cccccc" height=22><td nowrap>INV No.</td><td nowrap>{$form.inv_no}</td></tr>
		{if $config.do_approval_by_department and $form.dept_name}
		    <tr height=22><td nowrap>Department</td><td nowrap>{$form.dept_name|default:'&nbsp;'}</td></tr>
		{/if}
	...
	{else}
		...
		<tr>
			<td valign=top width=50% style="border:1px solid #000; padding:5px">
				...
				{/if}
				<br>
				{* if $to_branch.gst_register_no and $form.is_under_gst}GST No: {$to_branch.gst_register_no}<br />{/if *}
...
{foreach from=$do_items key=item_index item=r name=i}
	...
	<tr class="no_border_bottom {if $smarty.foreach.i.iteration eq $PAGE_SIZE and !$is_lastpage}td_btm_got_line{/if}">
		...
		{if !$page_item_info.$item_index.not_item}
			...
			{* if $show_invoice}
				{assign var=discount_amt value=$cost_price*$r.item_discount/100}
				{assign var=cost_price value=$cost_price-$discount_amt}
			{/if *}
...
{if $config.invoice_footer}<p class="small" align="center">{$config.invoice_footer}</p>{/if}
