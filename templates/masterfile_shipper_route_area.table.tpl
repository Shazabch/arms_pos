...
- new form: transporter route_area
*}
...
<table class="sortable" id="trnsprt_tbl" border=0 cellpadding=4 cellspacing=1 width=100%>
	<tr>
		...
		<th bgcolor={#TB_COLHEADER#}>Route Name</th>
		<th bgcolor={#TB_COLHEADER#}>Area</th>
		<th bgcolor={#TB_COLHEADER#}>Sequence</th>
	...
	<tr onmouseover="this.bgColor='{#TB_ROWHIGHLIGHT#}';" onmouseout="this.bgColor='';">
		<td bgcolor={#TB_ROWHEADER#} nowrap>
			<a href="javascript:void(open_route_area('{$r.id}'))"><img src=ui/ed.png title="Edit" border=0></a>
		...
		<td>
		<b>{$r.route_name}</b>{if !$r.active}<br><span class=small>(inactive)</span>{/if}
		</td>
		<td>{$r.area}</td>
		<td>{$r.sequence}</td>
		<td>{$r.added}</td>
