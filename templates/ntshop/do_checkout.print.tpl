...
4/25/2016 11:54 AM Andy
- Change "Code" to "Article".
- Change "Article/Mcode" to "MCode".
*}
...
<tr class="no_border_bottom {if $smarty.foreach.i.iteration eq $PAGE_SIZE and !$is_lastpage}td_btm_got_line{/if}">
	...
	<td norwap>{$r.artno|default:'&nbsp;'}</td>
	<td nowrap>{$r.mcode|default:'&nbsp;'}</td>
	<td width="50%"><div class="crop">{$r.description}</div></td>
	...
	{else}
		{if !$hide_RSP}<td>&nbsp;</td>{/if}<td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>	
	{/if}
</tr>
{/foreach}
