...
<tr bgcolor="#cccccc">
	...
	{if $form.do_type ne "transfer"}
		<th width="5%" nowrap>NRIC</th>
		<th width="8%" nowrap>Name</th>
		<th width="20%" nowrap>Address</th>
		<th width="5%" nowrap>Contact No</th>
		<th width="5%" nowrap>Email</th>
...
{if $form.do_type ne "transfer"}
	{foreach from=$do_items item=item key=r}
		...
		<tr class="no_border_bottom {if $smarty.section.i.iteration eq $PAGE_SIZE and !$is_lastpage}td_btm_got_line{/if}">
			...
			<td align="center" nowrap>{$item.serial_no.nric.$snr|default:'&nbsp;'}</td>
			<td nowrap>{$item.serial_no.name.$snr|default:'&nbsp;'}</td>
			<td nowrap>{$item.serial_no.address.$snr|default:'&nbsp;'}</td>
			<td align="center" nowrap>{$item.serial_no.cn.$snr|default:'&nbsp;'}</td>
			<td align="center" nowrap>{$item.serial_no.email.$snr|default:'&nbsp;'}</td>
