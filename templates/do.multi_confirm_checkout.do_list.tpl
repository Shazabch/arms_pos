...
<div class="card mx-3">
	<div class="card-body">
		<div class="table-responsive">
			<table width="100%"  class="report_table table mb-0 text-md-nowrap  table-hover" id="tbl_do_list">
					...
					<tbody class="fs-08">
						<tr id="tr_item-{$do.id}-{$do.branch_id}" class="tr_item {if $do_err_list.$bid.$do_id}highlight_row{/if}">
							...
							{if $config.consignment_modules && is_array($config.masterfile_branch_region) && is_array($config.consignment_multiple_currency)}
								<td align="center">{$do.total_foreign_amount|number_format:2}</td>
							{/if}
							
							{if $config.do_transfer_have_discount || $config.do_cash_sales_have_discount || $config.do_credit_sales_have_discount}
								<td align="center">{$do.total_inv_amt|number_format:2}</td>
								{if $config.consignment_modules && is_array($config.masterfile_branch_region) && is_array($config.consignment_multiple_currency)}
									<td align="center">{$do.total_foreign_inv_amt|number_format:2}</td>
								{/if}
							{/if}
							<td align="center">
