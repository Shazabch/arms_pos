...
&nbsp;&nbsp;
<a href="javascript:void(do_sort_type('*', 0));" id="items_lst1" {if $curr_sku_type eq '*' or $curr_sku_type eq ''}class=active{/if}>ALL</a>
{assign var=tab_count value=2}
{foreach from=$sku_type_list key=st_code item=st}
	<a href="javascript:void(do_sort_type('{$st_code}', 0))" id="items_lst{$tab_count}" {if $curr_sku_type eq $st_code}class=active{/if}>{$st.description|strtoupper}</a>
