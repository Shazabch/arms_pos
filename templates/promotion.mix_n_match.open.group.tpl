...
<div class="card mx-3">
	<div class="card-body">
		<div class="stdframe div_promo_group"  id="div_promo_group-{$group_id}">
		   ...
		   {if $allow_edit}
			   <button class="btn btn-primary mb-2" onClick="MIX_MATCH_MAIN_WIZARD_DIALOG.open('{$group_id}');">
		   ...
		   <table>
			   ...
			   <tr>
				   <td>
					   <b class="form-label">Limit qty discount per receipt /
						   <br />
				   ...
				   <td>
					   <input class="form-control" type="text" name="receipt_limit[{$group_id}]" value="{$promo_group.header.receipt_limit}" size="5" onChange="miz(this);check_group_limit('{$group_id}');" />
			   ...
			   <!-- Discount Preference -->
			   <tr>
				   <td><b class="form-label">Discount Preference</b></td>
				   <td>
					  <div class="form-label">
					   ...
					   <span style="color:blue;">
						   <small class="fs-08">(This will not affect those discount qty with "All Items", "Group Total".)</small>
					   </span>
				   </td>
			   </tr>
			   ...
			   <tr>
				   <td><b class="form-label">Discount Follow Item Sequence</b></td>
				   <td>
			   ...
			   <tr>
				   <td><b class="form-label">Prompt When Available</b>
			   ...
			   <tbody id="tbody_member_settings-{$group_id}" style="{if (!$got_member && !$is_first) || $config.promotion_hide_member_options}display:none;{/if}">
				   ...
				   <tr>
					   <td valign="top"><b class="form-label">Override Member Reward Point by Group</b>
						   
					   </td>
					   <td>
						   {if $sessioninfo.privilege.MEMBER_POINT_REWARD_EDIT}
							   <select class="form-control" name="item_category_point_inherit_data[{$group_id}][inherit]" onChange="item_category_point_inherit_changed('{$group_id}');">
							  ...
							  <b class="form-label">
							   ...
							   <table class="report_table">
								   ...
								   <tr>
									   ...
									   <td>
										   <input class="form-control" type="text" name="item_category_point_inherit_data[{$group_id}][global]" value="{$promo_group.header.item_category_point_inherit_data.global}" size="3" onChange="category_point_value_changed(this);" {if !$sessioninfo.privilege.MEMBER_POINT_REWARD_EDIT}readonly{/if} />
									   </td>
								   </tr>
								   {foreach from=$config.membership_type key=mtype item=mtype_desc name=fmt}
									   {if is_numeric($mtype)}
									   ...
									   <tr>
										   ...
										   <td>
											   <input class="form-control" type="text" name="item_category_point_inherit_data[{$group_id}][{$mt}]" size="3" onChange="category_point_value_changed(this)" value="{$promo_group.header.item_category_point_inherit_data.$mt}" {if !$sessioninfo.privilege.MEMBER_POINT_REWARD_EDIT}readonly{/if} />
										   </td>
									   </tr>
								   {/foreach}
							   </table>
				   ...
				   <tr id="tr_control_type-{$group_id}">
					   <td><b class="form-label">Control Type</b></td>
					   <td>
						  <div class="form-inline">
							<select class="form-control" name="control_type[{$group_id}]">
		   ...
		   <a href="javascript:void(MIX_MATCH_MAIN_MODULE.toggle_group_overlap_promo('{$group_id}', 0));">
			   <img src="/ui/collapse.gif" /> Hide all overlap promotion
				  ...
				  <div class="table-responsive">
					<table width=100% class="table mb-0 text-md-nowrap  table-hover"   border="0" cellspacing="1" cellpadding="1" id="tbl_promo_group_items-{$group_id}">
						...
						<tbody class="fs-08" id="tbody_promo_group_items-{$group_id}">
		   ...
		   <p>
			   {if $allow_edit}
				   <button class="btn btn-info mt-2" onClick="search_new_promo_item('{$group_id}');" id="btn_search_new_discount_item-{$group_id}">
					   <img src="/ui/findcat_expand.png" align="absmiddle" />
					   Search Discount Item
				   </button>
				   <button class="btn btn-primary mt-2" onClick="add_new_receipt_discount_item('{$group_id}');" id="btn_add_receipt_discount-{$group_id}">
					   <img src="/ui/inote16.png" align="absmiddle" />
					   Add Receipt Discount
				   </button>
				   <button class="btn btn-primary mt-2" onClick="add_new_special_foc_item('{$group_id}');" id="btn_add_special_foc-{$group_id}">
