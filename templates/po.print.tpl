...
4/20/2018 11:20 AM Andy
- Added Foreign Currency feature.

10/25/2018 12:07 PM Andy
- remove cellpadding=0 in items table.

10/26/2018 6:00 PM Andy
...
11/26/2018 10:55 AM Justin
- Bug fixed on showing 2 times of Art No/MCode.

12/17/2018 3:06 PM Justin
- Enhanced to print barcode on top of the document number.

5/21/2019 11:13 AM Andy
- Fixed barcode and po_no position overlap in other browser and pdf.

6/19/2019 10:16 AM William
- Added new Vertical print format.

7/25/2019 2:44 PM William
- Changed "deliver to" address use the branch deliver to address, if not set then use the default address.
*}
	...
	<td colspan="2" width="100%">
		<table width="100%" style="text-align: center;">
			...
			{if $encoded_comp_logo}
				<td><img style="max-width: 600px;max-height: 80px;" src="data:image/gif;base64,{$encoded_comp_logo}" height=80 hspace=5 vspace=5></td>
			{else}
				<td><img style="max-width: 600px;max-height: 80px;" src="{get_logo_url mod='po'}" height=80 hspace=5 vspace=5></td>
			{/if}
			</tr>
			{if $system_settings.verticle_logo_no_company_name neq 1}
			<tr>
				<td><h2>{$billto.description} {if $billto.company_no}({$billto.company_no}){/if}</h2></td>
			</tr>
			{/if}
			<tr>
...
{else}
	...
	<td width=100%>
	<h2>{$billto.description} {if $billto.company_no}({$billto.company_no}){/if}</h2>
...
{/if}
		...
		<tr bgcolor="#eeeeee">
			<td nowrap>PO No.</td>
			<td {if $form.approved eq 1 && $config.print_document_barcode}align="center" style="padding:0;line-height:23px;"{/if} nowrap>
...
<td colspan=2>
	...
	<tr>
		...
		{if $deliver.deliver_to}
			{$deliver.deliver_to|nl2br}
		{else}
			{$deliver.address|nl2br}
		{/if}
		<br>
...
<table width=100% cellspacing=0 border=0 class="box small tb">
<tr class="hd topline">
	<th rowspan=2>No</th>
	<th rowspan=2 nowrap>
		Art/MCode<br>
	...
	<th rowspan="2">Cost
		{if $form.currency_code}<br />({$form.currency_code}){/if}
	</th>
	<th rowspan=2>UOM</th>
	<th colspan=2>Qty</th>
	<th colspan=2>FOC</th>
...
<tr height=30 class="rw{cycle name=row values=",2"} no_border_bottom">
	{if !$page_item_info.$item_id.not_item}
		...
		<td nowrap>{if $item.artno}{$item.artno|default:"&nbsp;"}{else}{$item.mcode|default:"&nbsp;"}{/if}<br>
			{if $config.replace_docs_arms_code_with_link_code}
	...
	<tr class="ft2 topline">
		<td colspan="{$famt_colspan}" align=right>
			...
			{if $form.currency_code}
				{* $config.foreign_currency[$form.currency_code].symbol *}
				{$form.currency_code}
			{else}
	...
	{if $form.is_under_gst}
		...
		<tr class="ft2 topline">
			<td colspan="{$famt_colspan}" align=right>
				...
				{$config.arms_currency.symbol} {$form.supplier_po_amt_incl_gst|number_format:2}
				</h1>
			</td>
		</tr>
	{/if}
...
{if $is_lastpage}
	...
	<tr>
	<td width=40% valign=top class=xsmall>
	<b>IMPORTANT:</b><br>
...
<table width=100% cellspacing=0 cellpadding=0 border=0 class="small">
<tr>
	<td rowspan="2" valign=bottom width="100%">
		<h2>{$billto.description} {if $billto.company_no}({$billto.company_no}){/if}</h2>
...
{/if}
		...
		{if $form.currency_code}
			<tr>
				<td nowrap>Currency [{$form.currency_code}]</td>
				<td nowrap>Rate [{$form.currency_rate}]</td>
			</tr>	
		{/if}
...
<tr class="hd topline">
	...
	<th nowrap style="background:#aaa">GP{if $form.currency_code}*{/if}</th>
	{if $form.is_under_gst}
		<th rowspan=2>GST</th>
		<th rowspan=2>Amount <br />Include GST</th>
	{/if}
...
<tr height=15 class="rw{cycle name=crow values=",2"} no_border_bottom">
	{if !$page_item_info.$item_id.not_item}
		...
		<td rowspan=2 nowrap>
			{if $item.artno}{$item.artno|default:"&nbsp;"}{else}{$item.mcode|default:"&nbsp;"}{/if}<br>
			{if $config.replace_docs_arms_code_with_link_code}
				{$item.link_code|default:'&nbsp;'}
			{else}
				{$item.sku_item_code}
	...
	{if !$page_item_info.$item_id.not_item}
		...
		{else}
			{math assign=total_profit equation="round(selling - (nett_amt * currency_rate),2)" selling=$item.item_total_selling nett_amt=$item.item_nett_amt currency_rate=$form.currency_rate|default:1}
		{/if}
		<td nowrap align=right style="background:{$rowbg}; border-top:1px solid #000">{$total_profit|number_format:2}{if $form.currency_code}*{/if}</td>
		{if $form.is_under_gst}
			{if $item.is_foc}
				<td rowspan=2>&nbsp;</td>
				<td rowspan=2>&nbsp;</td>
...
{if $is_lastpage}
	...
	<tr height=30>
		...
		{math assign=total_profit equation="round(selling - (nett_amt * currency_rate),2)" selling=$form.total_selling_amt nett_amt=$form.subtotal_po_nett_amount currency_rate=$form.currency_rate|default:1}
		<td class="ft" nowrap align=right>{$form.subtotal_po_nett_amount|number_format:2}</td>
		<td class="ft" nowrap align=right>{$total_profit|number_format:2}{if $form.currency_code}*{/if}</td>
		{if $form.is_under_gst}
	...
	<!-- Actual PO Amount -->
	<tr class="ft2 topline">
		<td colspan="9" align="right"><b>Actual PO Amount{if $form.currency_code} ({$form.currency_code}){/if}</b></td>
		<td nowrap align="right">
			{$form.po_amount|number_format:2}
		</td>
		{math assign=final_profit equation="round(selling - (nett_amt * currency_rate),2)" selling=$form.total_selling_amt nett_amt=$form.po_amount currency_rate=$form.currency_rate|default:1}
		<td align=right>{$final_profit|number_format:2}{if $form.currency_code}*{/if}</td>
		{if $form.is_under_gst}
			<td>&nbsp;</td>
	...
	<!-- Supplier PO Amount -->
	<tr class="ft2 topline">
		<td colspan="9" align="right"><b>Supplier PO Amount{if $form.currency_code} ({$form.currency_code}){/if}</b></td>
		<td nowrap align="right">
			{$form.supplier_po_amt|number_format:2}
		</td>
		<td>&nbsp;</td>
