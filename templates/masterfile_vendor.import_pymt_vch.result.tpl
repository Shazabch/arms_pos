...
<p style="color:blue;">
	...
	* Please ENSURE the result data is fill to the header accordingly before proceed to import.<br />
	<br/>
	<input type="button" id="import_btn" name="import_btn" value="Import" onclick="IMPORT_VENDOR_PYMT_VCH.import_pymt_vch({$method});" {if !$result.import_row}disabled{/if} />
</p>
<div class="div_tbl">
	<table id="si_tbl" width="100%">
		...
		<tbody>
		{foreach from=$item_lists item=i name=vpv}
			<tr class="{if $i.error}tr_error{/if}">
				<td>{$smarty.foreach.vpv.iteration}.</td>
