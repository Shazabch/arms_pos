...
6/28/2012 4:10:23 PM Justin
- Enhanced to show certain information only while print GRN from waiting for approval.

7/2/2012 1:52:55 PM Justin
- Enhanced to have show 2 copies, vendor and account.
- Fixed the page footer shown in next page.
...
11/20/2012 5:51 PM Justin
- Added missing vendor code on account copy.
...
3/17/2016 10:06 AM Qiu Ying
- Fix numbering shown in SKU Additional Description 

4/21/2017 11:35 AM Khausalya
...
<td>{$grr.prompt_payment_term|default:"&nbsp;"}</td>
{if $grr.type eq 'PO'}
...
{if !$is_gra}
	...
	* GRN Balance is Rcv Qty minus total sales and DO from {$grr.rcv_date|date_format:"%d/%m/%Y"} to {$smarty.now|date_format:"%d/%m/%Y"}.
	</div>
{/if}
...
{if $is_gra}
	...
	<tr class="topline botline" bgcolor=#cccccc>
		...
		<th rowspan=2 width='15'>Rcv Qty</th>
		{if !$is_gra}
			<th colspan=2 width='40'>* S.Price({$config.arms_currency.symbol})</th>
		{/if}
		<th colspan=2 width='40'>Cost ({$config.arms_currency.symbol})</th>
		{if !$is_gra}
			<th colspan=3 width='40'>{if $grr.type eq 'PO' || !$grr.is_ibt_do}PO{else}DO{/if} Details</th>
			<th rowspan=2 width='20'>GP (%)</th>
			<th colspan=2 width='35'>GRN Balance</th>
	...
	<tr height=30 bgcolor="{cycle name=r1 values=",#eeeeee"}" class="no_border_bottom">
		...
		{if !$page_item_info.$iid.not_item}
			...
			{if !$is_gra}
				...
				{if $qty}
					{assign var=s_c value=`$price-$cost`}
					{assign var=gp value=`$s_c/$price*100`}
				{else}
					{assign var=gp value=0}
				{/if}
				<td align=right>{$gp|number_format:2}</td>
