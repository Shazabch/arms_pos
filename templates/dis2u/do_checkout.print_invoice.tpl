...
8/13/2013 2:43 PM Justin
- Enhanced to show weight and total weight.
...
<tr bgcolor=#cccccc>
	...
	<th nowrap rowspan="2" width="40">Weight<br />(KG)</th>
...
{foreach from=$do_items key=item_index item=r name=i}
	...
	<tr class="no_border_bottom {if $smarty.foreach.i.iteration eq $PAGE_SIZE and !$is_lastpage}td_btm_got_line{/if}">
		...
		{assign var=cost_price value=$r.cost_price}
			...
			<td align="right">{$r.weight|number_format:2}</td>
			...
			{assign var=total_weight value=$r.weight+$total_weight}
		{else}
...
{if $is_lastpage}
	...
	{if $show_invoice and $form.discount}
	    ...
	    <tr class="total_row">
	        ...
	        <th align="right">{$form.discount|default:'-'}</th>
	        <th align="right">{$discount_amt|number_format:2}</th>
	    </tr>
	{/if}

	{assign var=cols value=6}
	{if $show_invoice}{assign var=cols value=$cols+1}{/if}
	{assign var=total_rounded_inv_amount value=$total|rounding}
	<tr class="total_row">
		<th align=right colspan="{$cols}" class="total_row">Total {if $form.do_type eq 'open' && $config.do_enable_cash_sales_rounding && $total_rounded_inv_amount ne $total}Before Round{/if}</th>
		<th align=right class="total_row">{$total_weight|number_format:2}</th>
		<th align=right class="total_row">{if !$config.do_enable_cash_sales_rounding || $total_rounded_inv_amount eq $total}{$total_ctn|qty_nf}{else}&nbsp;{/if}</th>
		<th align=right class="total_row">{if !$config.do_enable_cash_sales_rounding || $total_rounded_inv_amount eq $total}{$total_pcs|qty_nf}{else}&nbsp;{/if}</th>
		<th align=right class="total_row">{$total|number_format:2}</th>
	</tr>

	{if $form.do_type eq 'open' && $config.do_enable_cash_sales_rounding && $total_rounded_inv_amount ne $total}
		<tr class="total_row">
			<th align=right colspan="{$cols}" class="total_row">Rounding</th>
			<th align=right class="total_row">&nbsp;</th>
			<th align=right class="total_row">&nbsp;</th>
			<th align=right class="total_row">&nbsp;</th>
			<th align=right class="total_row">{$total_rounded_inv_amount-$total|number_format:2}</th>
		</tr>
		<tr class="total_row">
			<th align=right colspan="{$cols}" class="total_row">Total After Rounded</th>
			<th align=right class="total_row">{$total_weight|number_format:2}</th>
			<th align=right class="total_row">{$total_ctn|qty_nf}</th>
			<th align=right class="total_row">{$total_pcs|qty_nf}</th>
			<th align=right class="total_row">{$total_rounded_inv_amount|number_format:2}</th>
		</tr>
	{/if}
