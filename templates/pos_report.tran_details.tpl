...
&nbsp;&nbsp;&nbsp;&nbsp;
<b>Counter</b> <select name="counters">
...
<input type="text" name="receipt_no" value="{$smarty.request.receipt_no}" />
...
<p>
<b>Payment Type</b> <select name="payment_type">
<option value="all">-- All --</option>
...
<b>Transaction Status</b> <select name="tran_status">
<option value="all">-- All --</option>
...
<b>Transaction Type</b> <select name="tran_type">
<option value="all">-- All --</option>
...
<b>Transaction Filter</b> <select name="other_filter">
...
<p>
<b>Receipt Amount</b>
<select name="receipt_amt_type" onchange="POS_TRAN_DETAILS.receipt_amt_type_changed(this);">
...
<span id="default_receipt_amt" {if $smarty.request.receipt_amt_type eq 'between'}style="display:none;"{/if}>
	<input type="text" name="default_receipt_amt_val" value="{$smarty.request.default_receipt_amt_val}" onchange="mf(this);" />
</span>
<span id="between_receipt_amt" {if $smarty.request.receipt_amt_type ne 'between'}style="display:none;"{/if}>
	<input type="text" name="min_receipt_amt_val" value="{$smarty.request.min_receipt_amt_val}" onchange="mf(this);" />
	&nbsp;To&nbsp;
	<input type="text" name="max_receipt_amt_val" value="{$smarty.request.max_receipt_amt_val}" onchange="mf(this);" />
</span>
</p>
{include file='sku_items_autocomplete_multiple_add2.tpl' parent_form='document.myForm'}
<input type="button" value="{#SHOW_REPORT#}" onClick="POS_TRAN_DETAILS.show_report();" />
{if $table}
...
{foreach from=$table key=bid item=p}
	<h3>{$branches.$bid}: <span id="span_branch_pos_count-{$bid}">{count var=$p}</span> transaction(s)</h3>
	<table width="100%" class="sortable report_table" id="tbl_pos_list-{$bid}">
		<thead>
			<tr class="header">
				<th>No.</th>
				<th>Receipt No.</th>
		...
		<tfoot class="sortbottom tfoot_pos_list" id="tfoot_pos_list-{$bid}">
