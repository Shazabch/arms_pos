...
<tr bgcolor=#cccccc>
	...
	<th width=40 align="right">U.Price<br>(RM)</th>
	<th width="40">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</th>
	<th width=40>Total Amount<br>(RM)</th>
</tr>
...
<tr class="no_border_bottom {if $smarty.section.i.iteration eq $PAGE_SIZE and !$is_lastpage}td_btm_got_line{/if}">
	...
	<td nowrap class="artno_col">{if $do_items[i].artno <> ''}{$do_items[i].artno}{else}{$do_items[i].mcode|default:'&nbsp;'}{/if}</td>
	...
	{if !$page_item_info[i].not_item}
		...
		{assign var=cost_price value=$do_items[i].cost_price}
		
		<td align="right" width="5%">{$cost_price|number_format:2}</td>
		{assign var=row_amt value=$cost_price*$do_items[i].pcs}
		<td>&nbsp;</td>
		<td align="right" width="5%">{$row_amt|number_format:2}</td>
...
<tr>
	{if $form.sheet_price_type}
		...
		{/if}
		<td nowrap style="border:1px solid gray;border-right:1px solid gray !important;" colspan=3>{$form.sheet_price_type} = {$price_type}</td>
	{else}
		<td style="border:none;" colspan=3></td>
	{/if}
</tr>
