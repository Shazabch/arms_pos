...
{if $err}
	<h5>{$err}</h5>
{elseif !$sku_items}
	<h5>This vendor is from previous month. No item record for this month.</h5> 
{else}
	<table class="report_table">
		<tr class="header">
			...
			<th>&nbsp;</th>
			<th>System Stock Opening</th>
			{assign var=col1 value="System Stock Opening"}
			<th>GP (%)</th>
			{assign var=colgp value="GP (%)"}
			<th>Stock Check</th>
			{assign var=col2 value="Stock Check"}
			<th>GP (%)</th>
		...
		{foreach from=$sku_items key=sid item=other}
			{if $item_vos.$sid.cost_price || $item_vos.$sid.selling_price || $item_vstv.$sid.cost_price || $item_vstv.$sid.selling_price || $item_vsr.$sid.cost_price || $item_vsr.$sid.selling_price || $item_vadj.$sid.cost_price || $item_vadj.$sid.selling_price || $item_vrs.$sid.cost_price || $item_vrs.$sid.selling_price || $item_vpa.$sid.cost_price || $item_vpa.$sid.selling_price || $item_vpca.$sid.cost_price || $item_vpca.$sid.selling_price || $item_vacs.$sid.cost_price || $item_vacs.$sid.selling_price}
				...
				{if $sku_type == 'FRESH'}
					<tr>
						...
						<td>CP</td>
						<td title="{$col1}|{$item_descrip}" class="r">{$item_vos.$sid.selling_price|number_format:2|ifzero:'-'}</td>
						<td title="{$colgp}|{$item_descrip}">&nbsp;</td>
						<td title="{$col2}|{$item_descrip}" class="r">{$item_vstv.$sid.selling_price|number_format:2|ifzero:'-'}</td>
						<td title="{$colgp}|{$item_descrip}">&nbsp;</td>
				...
				{else}
					<tr>
						...
						{if $sku_type == 'OUTRIGHT'}
							<td title="{$col1}|{$item_descrip}" class="r">{$item_vos.$sid.cost_price|number_format:2|ifzero:'-'}</td>
							{assign var=costprice value=$item_vos.$sid.cost_price}
							{assign var=sellingprice value=$item_vos.$sid.selling_price}
							...
							<td title="{$col2}|{$item_descrip}" class="r">{$item_vstv.$sid.cost_price|number_format:2|ifzero:'-'}</td>
							{assign var=costprice value=$item_vstv.$sid.cost_price}
							{assign var=sellingprice value=$item_vstv.$sid.selling_price}
							{if $sellingprice ne 0 or $sellingprice ne ''}
								{assign var=gpvalue value=$sellingprice-$costprice}
								{assign var=gpvalue value=$gpvalue/$sellingprice}
							{else}
						...
						{elseif $sku_type == 'CONSIGN'}
							<td title="{$col1}|{$item_descrip}">&nbsp;</td>
							<td title="{$colgp}|{$item_descrip}" rowspan=2>&nbsp;</td>
							<td title="{$col2}|{$item_descrip}">&nbsp;</td>
							<td title="{$colgp}|{$item_descrip}" rowspan=2>&nbsp;</td>
							...
							<td title="{$colper}|{$item_descrip}" rowspan=2>&nbsp;</td>
							{assign var=sellingprice value=$item_vacs.$sid.selling_price}
							{assign var=costprice value=$sales_gp/100*$sellingprice}
							{assign var=costprice value=$sellingprice-$costprice}
							
							<td title="{$col8}|{$item_descrip}" class="r">{$costprice|number_format:2|ifzero:'-'}</td>	
							<td title="{$colgp}|{$item_descrip}" rowspan=2 class="r">{$sales_gp|number_format:2|ifzero:'-':$percent}</td>
						{/if}
					</tr>
					<tr>
						<th>SP</th>
