...
7/15/2011 1:52:01 PM Andy
- Add include 'header.print.tpl' for all printing templates, added support charset utf8.

8/9/2011 3:31:34 PM Andy
...
12/15/2011 10:46:43 AM Justin
- Added checking not to treat "RM" as foreign cost.

12/15/2011 10:54:49 AM Andy
- Added to show "Normal Price" for Cash Sales DO.
...
- Remove table line
- Remove ARMS code
- All performa invoice price use RM

8/19/2013 9:31 AM Fithri
- assign document no as filename when print

3/23/2015 2:54 PM Andy
...
- Bug fixed on colspan wrongly set for total rows.

5/13/2015 10:18 AM Andy
...
{assign var=foreign_currency_multiply value=1}
{if $form.do_type eq 'transfer' and $config.consignment_modules and $config.masterfile_branch_region and $config.consignment_multiple_currency and $form.exchange_rate>1}
	...
	{else}
		{assign var=foreign_currency_multiply value=1/$exchange_rate}
	{/if}
{/if}
...
<div style="background:#000;padding:4px;color:#fff" align=center><b>
    {if $form.do_type eq 'open'}Cash Bill{elseif $form.do_type eq 'credit_sales'}Credit Sales{else}CONSIGNMENT NOTE{/if}<br />
INVOICE</b></div>
...
<tr bgcolor=#cccccc class="td_btm_got_line">
	...
	{if $form.do_type eq 'open'}
		<th rowspan=2 width=40>Normal Price<br>({if $form.do_type eq 'transfer' && $to_branch.currency_code}{$to_branch.currency_code}{else}RM{/if})</th>
	{/if}
	{*<th rowspan=2 width=40>Price<br>({if $form.do_type eq 'transfer' && $to_branch.currency_code}{$to_branch.currency_code}{else}RM{/if})</th>*}
	...
	{*<th rowspan=2 width=80>Total Amount<br>({if $form.do_type eq 'transfer' && $to_branch.currency_code}{$to_branch.currency_code}{else}RM{/if})</th>*}
...
{foreach from=$do_items key=item_index item=r name=i}
	...
	<tr class="no_border_bottom {if $smarty.foreach.i.iteration eq $PAGE_SIZE and !$is_lastpage}td_btm_got_line{/if}">
		...
		{*<td align="center" nowrap>{$sku_item_code}</td>*}
		...
		{if !$page_item_info.$item_index.not_item}
			{*{if $form.do_type eq 'transfer' && $to_branch.currency_code && $to_branch.currency_code ne "RM" && $form.exchange_rate ne '1'}
				{assign var=cost_price value=$r.foreign_cost_price}
				{assign var=local_cost_price value=$r.cost_price}
			{else}
				{assign var=cost_price value=$r.cost_price}
			{/if}*}
			{assign var=cost_price value=$r.cost_price}

			<!-- normal price b4 markup/discount -->
			{if $form.do_type eq 'open'}
			...
			{assign var=local_cost_price value=`$p_markup*$local_cost_price` *}
			...
			{if $local_cost_price ne '0'}
				{assign var=lcl_amt_ctn value=$local_cost_price*$r.ctn}
				{assign var=lcl_amt_pcs value=$local_cost_price/$r.uom_fraction*$r.pcs}
				{assign var=local_total_row value=$lcl_amt_ctn+$lcl_amt_pcs}
			{/if}

			{if $show_invoice}
				{get_discount_amt assign=discount_amt amt=$total_row discount_pattern=$r.item_discount currency_multiply=$foreign_currency_multiply}
		...
		{else}
			{if $form.do_type eq 'open'}<td>&nbsp;</td>{/if}
			<td>&nbsp;</td>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
			<td>&nbsp;</td>
