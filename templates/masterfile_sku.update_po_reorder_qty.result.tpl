...
<p>
	...
	{assign var=curr_upd_type value=$form.update_type}
	<b>Update PO Reorder Qty by:&nbsp;&nbsp;{$upd_type_list.$curr_upd_type}</b>
	<input type="hidden" name="update_type" value="{$form.update_type}" />
	<div {if $form.update_type ne "branch"}style="display:none;"{/if}>
		<b>Branch Selected: </b>
	...
	<input type="button" id="update_btn" name="update_btn" value="Update" onclick="UPDATE_SKU_PO_REORDER_QTY_MODULE.update_sku(1);" {if !$result.updated_row}disabled{/if} />
</p>
<div class="div_tbl">
