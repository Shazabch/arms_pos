...
<form name="f_a" class="noprint stdframe" style="background-color:#fff" method="post" onSubmit="return false;">
	...
	<p>
		...
		<input type="button" value='Show Report' onClick="BRAND_REPORT.submit_form();" /> 
		<input type="button" value="Print" onclick="BRAND_REPORT.do_print();" />
		
		{if $sessioninfo.privilege.EXPORT_EXCEL}
			<input type=button value="Export to Excel" onclick="BRAND_REPORT.submit_form('excel');" />
		{/if}
	</p>
</form>
