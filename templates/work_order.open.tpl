				...
				<table width="100%">
					...
					<tr>
						...
						<div class="form-inline">
							<input class="form-control" name="adj_date" id="inp_adj_date" size="50" maxlength="10"  value="{$form.adj_date|date_format:"%Y-%m-%d"}" class="required" title="Adjustment Date" />
							{if $can_edit}
					...
					<tr>
						...
						<td>
							...
							{else}
								<input class="from-control" type="hidden" name="transfer_type" value="{$form.transfer_type}" />
							...
							<div class="stdframe" style="height:80px;width:200px;overflow-y:auto;background-color:#fff;">
								{foreach from=$form.notify_users_in_obj key=uid item=r}
									{if !$transferInUserList.$uid}
										<input type="checkbox" name="notify_users_in[{$uid}]" value="{$uid}" checked {if $action ne 'out'}onChange="this.checked=!this.checked;"{/if} /> <span class="">{$r.u}</span><br />
									{/if}
								{/foreach}
								
								{foreach from=$transferInUserList key=uid item=r}
								<div style="border-bottom: 1px solid gray;">
									<input type="checkbox" name="notify_users_in[{$uid}]" value="{$uid}" {if $form.notify_users_in.$uid}checked {/if} {if $action ne 'out'}onfocus="blur();"{/if} /> <span class="">{$r.u}</span><br />
								</div>
								{/foreach}
							</div>
							<br>
							<span class="mt-2">When complete, will send to Notify Users in Adjustment Approval Flow</span>
						</td>
					</tr>
				</table>
			</div>
	...
	<p id="p_submit_btn" align="center">
		{if $can_edit}
			...
			{if !is_new_id($form.id) and $action eq 'out'}
				<input type="button" value="Delete" class="btn btn-danger" onclick="WO_OPEN.form_delete_clicked()" />
			{/if}
			<input type="button" value="Close" class="btn btn-info" onclick="WO_OPEN.form_close_clicked();" />
			
			{if $action eq 'out'}
				<input type="button" value="Confirm (Send to Transfer In)" class="btn btn-success" onclick="WO_OPEN.form_confirm_clicked();" />
			{else}
				{if ($sessioninfo.level>=$config.doc_reset_level)}
				<input type="button" value="Reset to Transfer Out" class="btn btn-danger" onclick="WO_OPEN.form_reset_clicked('out');" />
				{/if}
				<input type="button" value="Complete" class="btn btn-success" onclick="WO_OPEN.form_confirm_clicked();" />
			{/if}
		{else}
			{if $form.branch_id eq $sessioninfo.branch_id and $form.active eq 1 and $form.status eq 1 and ($sessioninfo.level>=$config.doc_reset_level)}
				<input type="button" value="Reset to Transfer Out" class="btn bnt-danger" onclick="WO_OPEN.form_reset_clicked('out');" />
				{if $form.completed eq 1}
					<input type="button" value="Reset to Transfer In" class="btn btn-danger" onclick="WO_OPEN.form_reset_clicked('in');" />
				{/if}
			{/if}
			<input type="button" class="btn btn-info" value="Close"  onclick="WO_OPEN.form_close_clicked();">
		{/if}
	</p>
