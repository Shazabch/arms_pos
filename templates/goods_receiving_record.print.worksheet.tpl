...
4/16/2018 4:23 PM Justin
- Enhanced to load currency code base on GRR instead of hardcoded it.

10/30/2018 10:28 AM Justin
- Enhanced to show Branch Company Registration No. after company name.

5/22/2019 4:35 PM William
...
var doc_no = '{$branch.report_prefix}{$grr.grr_id|string_format:"%05d"}';
{literal}
...
<tr>
	...
	<td nowrap>{$branch.report_prefix}{$grr.grr_id|string_format:"%05d"}</td>
</tr>
<tr>
...
<tr class="" height=24 bgcolor=#cccccc>
	...
	<th width=40>UOM</th>
	{if $po.is_foc}
	<th width=40>GRN Cost<br>({$grr.currency_code|default:$config.arms_currency.symbol})</th>	
	{/if}
	{if $po.grn_po_qty}
	{*
	...
	<th>PO<br>Selling</th>
	*}
	<th nowrap>PO<br>Suggested<br>Selling({$grr.currency_code|default:$config.arms_currency.symbol})</th>
	<th width=40>Current<br>Selling<br>({$grr.currency_code|default:$config.arms_currency.symbol})</th>
	<th width=40>PO Cost<br>({$grr.currency_code|default:$config.arms_currency.symbol})</th>
	...
	<tr height=24 bgcolor="{cycle name=r1 values=",#eeeeee"}" class="no_border_bottom">
		...
		*}
			...
			<td align=right nowrap>
				...
				{if !$item.include_tax}<sup style="font-size:7px;">1</sup>{/if}
...
<p style="font-size: smaller;">
	<sup>#</sup> Not euqal to SKU Selling Price.<br>
	<sup>1</sup> Exclude tax.<br>
	<sup>*</sup> Lower than Cost.
</p>
{if $is_lastpage}
	<br>
