...
class MULTI_BRANCH_SALES extends Module{
	var $file_folder = "tmp/Multi_Branch_Sales";
	var $branches = array();
	...
	var $all_cat = false;
	var $group_by_sku = 0;
	var $tmp_si_tbl = "tmp_multi_branch_sales_list";
	...
	private function prepare_report_data(){
		...
		if($_REQUEST['date_from'] && $_REQUEST['date_to']){
			...
			if(strtotime($this->date_to) < strtotime($this->date_from)){
				$this->err[] = "Sales To must after Sales From";
			}else{
				$checking_date = strtotime("+3 months", strtotime($this->date_from));
				if (strtotime($this->date_to) >= $checking_date){
					$this->err[] = "Maximum Sales Date interval is 3 months";
				}
				$this->report_title[] = "Sales From: " . $this->date_from;
				$this->report_title[] = "Sales To: " . $this->date_to;
			}
		}else{
			if(!$_REQUEST['date_from']){
				$this->err[] = "Please select a Sales From";
			}
			
			if(!$_REQUEST['date_to']){
				$this->err[] = "Please select a Sales To";
			}
		}
		...
		if($this->err){
			...
			foreach($this->branch_id_list as $bid){
				$sql[] = "select si.$id_str, si.id as sku_item_id
				from sku_items_sales_cache_b$bid
			...
			$str_sql = implode(" Union All ",$sql);
			//$str_sql2 = implode(" Union All ",$sql2);
			
			$sql_count = "select count(distinct tbl.$id_str) as c
			from ($str_sql) as tbl
	...
	private function load_report(){
		...
		if($this->export_excel || $this->show_grand_total){
			...
			if($this->export_excel){	// excel only load current page data
				$filter_sql = " where stmp.row_no between $start and $end";
			}			
		}
		...
		$filter = $filter ? ' and ' . implode(' and ', $filter) : "";
		...
		$sql = "select si.id as sid,si.sku_id,si.is_parent, si.sku_item_code,si.mcode,si.artno,si.link_code,
			si.description,uom.code as uom_code, uom.fraction as uom_fraction, stmp.id as 'stmp_id', stmp.row_no
			from $this->tmp_si_tbl stmp
			left join sku_items si on stmp.id = $id_str
		...
		while($r = $con_multi->sql_fetchassoc($q1)){
			...
			$total_sales = 0;
			foreach($this->branch_id_list as $bid){
				$tmp_sql = "select $bid as bid, sum(qty) as qty, sum((qty*uom.fraction)) as uom_qty, sum(amount) as amount
				from sku_items_sales_cache_b$bid
				left join sku_items si on sku_item_id = si.id
				left join sku on sku.id=si.sku_id
				left join uom on uom.id=si.packing_uom_id
				left join category_cache cc on cc.category_id=sku.category_id
				where $id_str = " . ms($r["stmp_id"]) . "
				$filter
				group by $id_str";
				
				$q2 = $con_multi->sql_query($tmp_sql);
				while($r2 = $con_multi->sql_fetchassoc($q2)){
					...
					if(!$this->show_grand_total || ($this->show_grand_total && $r['row_no'] <= $end && $r['row_no'] >= $start)){
						$this->data['data']['by_item'][$key]['sales_by_branch'][$bid]['sales'] = $r2['amount'];
						$this->data['data']['by_item'][$key]['sales_by_branch'][$bid]['qty'] = $qty;
					
					
						$total_sales += $this->data['data']['by_item'][$key]['sales_by_branch'][$bid]['sales'];
						$total_qty += $this->data['data']['by_item'][$key]['sales_by_branch'][$bid]['qty'];
					}
					
					if($this->show_grand_total){   //store into array to avoid duplicate calculate
						$this->data['tmp_grand_total'][$bid][$key]['total_qty'] = $qty;
						$this->data['tmp_grand_total'][$bid][$key]['total_sales'] = $r2['amount'];
					}
				}
			...
			if(!$this->show_grand_total || ($this->show_grand_total && $r['row_no'] <= $end && $r['row_no'] >= $start)){
				if($need_record_si_info){
					if(!isset($this->data['si_info'][$key])){
						...
						$this->data['si_info'][$key]['total_qty'] = $total_qty;
						$this->data['si_info'][$key]['total_sales'] = $total_sales;
					}			
				}
				
				if($need_add_into_data){
					$this->data['tmp_page_total'][$key]['total_qty'] = $total_qty;
					$this->data['tmp_page_total'][$key]['total_sales'] = $total_sales;
				}
			}
		}
		$con_multi->sql_freeresult($q1);
		
		if($this->data['tmp_page_total']){
			foreach($this->data['tmp_page_total'] as $key=>$val){
				$this->data['page_total']['total_qty'] += $val['total_qty'];
				$this->data['page_total']['total_sales'] += $val['total_sales'];
			}
			unset($this->data['tmp_page_total']);
		}
		
		if($this->show_grand_total){
			if($this->data['tmp_grand_total']){
				foreach($this->data['tmp_grand_total'] as $bid=>$item_list){
					foreach($item_list as $key=>$val){
						$this->data['grand_total']['total_qty'] += $val['total_qty'];
						$this->data['grand_total']['total_sales'] += $val['total_sales'];
					}
				}
				unset($this->data['tmp_grand_total']);
			}
	...
	private function generate_excel(){
		...
		for($i = 0; $i <$this->data['total_page']; $i++){
			...
			file_put_contents($this->file_folder."/".$file_name, ExcelWriter::GetHeader().$smarty->fetch('report.multi_branch_sales.table.tpl').ExcelWriter::GetFooter());
		}
		
		$parent_zip = "Multi_Branch_Sales_".$sessioninfo['id']."_".$times;
	...
	function __construct($title){
		...
		if(!$_REQUEST['date_from'] && !$_REQUEST['date_to']){
			$_REQUEST['date_from'] = date("Y-m-d", strtotime("-7 day"));
...
$MULTI_BRANCH_SALES = new MULTI_BRANCH_SALES('Multi Branch Sales Report');
?>