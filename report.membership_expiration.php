...
class MEMBERSHIP_EXPIRATION extends Module{
	...
	private function run_report($bid){
		...
		while($r1 = $con_multi->sql_fetchassoc($q1)){
			...
			$q2 = $con_multi->sql_query("select count(receipt_no) as trans_count, sum(point) as points
										 from pos p
    ...
    function generate_report(){
		...
		if($this->trans_count_from || $this->trans_count_to){
			foreach($this->table as $row=>$r){
				if(($this->trans_count_from && $this->trans_count_from > $r['trans_count']) || ($this->trans_count_to && $this->trans_count_to < $r['trans_count'])) unset($this->table[$row]);
			}
		}
		
		$this->report_title[] = "Date From ".strtoupper($this->date_from)." to ".strtoupper($this->date_to);
		
		if(!$this->trans_count_from && !$this->trans_count_to) $tc_desc = "All";
		else{
			if($this->trans_count_from && !$this->trans_count_to) $tc_desc = "Start From ".$this->trans_count_from;
			elseif(!$this->trans_count_from && $this->trans_count_to) $tc_desc = "At Most ".$this->trans_count_to;
			else $tc_desc = "From ".$this->trans_count_from." To ".$this->trans_count_to;
		}
		$this->report_title[] = "Transaction Count: ".$tc_desc;
	...
	function process_form(){
		...
		$this->trans_count_from = $_REQUEST['trans_count_from'];
		$this->trans_count_to = $_REQUEST['trans_count_to'];
	...
	function ajax_show_member_details(){
		...
		$q1 = $con_multi->sql_query("select p.*, b.code as branch_code, u.u as cashier_name
									 from pos p
									 ...
									 order by p.date, p.pos_time, p.receipt_no");
		...
		$smarty->display("report.membership_expiration.detail.tpl");
	}
}

$MEMBERSHIP_EXPIRATION = new MEMBERSHIP_EXPIRATION('Membership Expiration Report');
?>
