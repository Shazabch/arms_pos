...
class COUNTER_COLLECTION_DETAILS_REPORT extends Module{
	...
	function __construct($title, $template=''){
	 ...
	 function _default(){
	 	...
	 	$this->display('aneka/pos_report.counter_collection_details.tpl');
	 }
	 ...
	 private function load_report(){
	 	...
	 	// must finalized
	 	$con->sql_query("select * from pos_finalized where branch_id=$this->branch_id and date=".ms($this->date)." and finalized=1");
	 	if(!$con->sql_numrows())	$err[] = "Counter Collection must finalised first.";
	 	$con->sql_freeresult();
	...
	private function generate_data($counter_id){
	 	global $con, $smarty, $pos_config,$mm_discount_col_value, $config;
	            ...
	            switch ($payment_type){
					...
					case 'deposit':	// pay by deposit
						if($pos['amount_change']){	// got amt changed
							...
							$con->sql_query("select count(*) from pos_payment pp where pp.branch_id=".mi($this->branch_id)." and pp.date=".ms($this->date)." and pp.counter_id=$counter_id and pp.pos_id=$pos_id and pp.type='Cash' and pp.adjust=0");
...
where pi.branch_id=".mi($this->branch_id)." and pi.date=".ms($this->date)." and pi.counter_id=$counter_id and pi.pos_id=$pos_id";
			...
			if($r['data']){
				...
				foreach($r['data'] as $type=>$d2){
                    ...
                    if($pos_config['currency']&&in_array($type_key, $pos_config['currency'])){  // is currency collection
                    	$currency_rate = $curr_rate[$type_key];
		...
		foreach(array('cashier_sales', 'cash_advance', 'cash_domination', 'variance','top_up') as $row_type){
			...
			if($this->data['foreign_currency_list']){
				foreach($this->data['foreign_currency_list'] as $currency_type=>$curr_data){
					$this->data['summary'][$counter_id][$row_type]['total_sales']['amt'] += round($this->data['summary'][$counter_id][$row_type][$currency_type]['amt'],2);
		...
		// drawer open count
		$con->sql_query("select count(*) from pos_drawer where branch_id=$this->branch_id and counter_id=".mi($counter_id)." and date=".ms($this->date)." and type = 'POS'");
		$this->data['summary'][$counter_id]['drawer_open_count'] = $con->sql_fetchfield(0);
		$con->sql_freeresult();
