<?php
/*
12/29/2009 10:17:27 AM Andy
- Fix show by all category bugs

2/23/2010 2:41:22 PM Andy
- Add option title when show report

5/6/2010 1:33:54 PM Alex
- Add HQ Cost

8/13/2010 10:05:21 AM Andy
- Hide Item with "SKU without inventory".

9/21/2010 9:56:31 AM Andy
- Fix wrong start qty.

10/26/2010 11:51:48 AM Alex
- add 1 day if filter with timestamp
- remove is_latest checking from query
- make both stock balance report tally

11/30/2010 12:01:48 PM Alex
- fix closing stock match to opening stock
- fix display max level of category

1/3/2011 4:32:33 PM Andy
- Fix report if show jan 1 opening will get zero stock balance.

1/14/2011 12:38:54 PM Alex
- add use_grn to get latest vendor 

1/21/2011 3:29:22 PM Alex
...
6/27/2011 10:00:03 AM Andy
- Make all branch default sort by sequence, code.

7/6/2011 2:45:11 PM Andy
- Change split() to use explode()

7/7/2011 10:18:18 AM Alex
- add export to excel

8/4/2011 12:25:28 PM Alex
- fix quantity figure bugs when show by vendor and category
...
9/20/2012 5:08 PM Andy
- Fix wrong stock take adjust.

9/27/2012 2:05:00 PM Fithri
- stock balance summary - add can show by branch

10/8/2012 5:40 PM Andy
- Reduce memory usage to improve the report ability to show all branch all category data.

11/20/2012 6:06 PM Justin
- Bug fixed on system show wrong info for title "Show By:".
...
8/25/2014 12:03 PM Fithri
- fix wrong calculation in Item List
*/
...
class Stock_Balance_Summary extends Module{
    function __construct($title){
		...
		// load items
		$con->sql_query("select bgi.*,branch.code,branch.description from branch_group_items bgi left join branch on bgi.branch_id=branch.id order by branch.sequence, branch.code",false,false);
		while($r = $con->sql_fetchrow()){
	...
	function _default(){
		$this->display("hasani/report.stock_balance_summary.tpl");
	}
	...
	function show_report(){
        ...
        foreach($bid_list as $bid){
	       ...
	       $sql = "select si.id as sid,si.id,si.sku_item_code ,si.artno,si.mcode,si.description,si.selling_price,c.id as category_id $ven_sql,sku.sku_type,sb1.qty as sb_from,
				...
				sku.category_id as real_cid,vendor.code as vcode,vendor.description as vname,c.tree_str,
			...
			while($r = $con_multi->sql_fetchassoc($sql_rid)){
				...
				//create item list
				if (abs($r['sb_to']) > 0) {
					$il['sku_item_code'] = $r['sku_item_code'];
					$il['mcode'] = $r['mcode'];
					$il['description'] = $r['description'];
					$il['qty'] = $r['sb_to'];
					$il['cost'] = $r['cost'];
					$il['total'] = ($r['sb_to']*$r['cost']);
					$item_list[] = $il;
					
					$item_list_total['qty_total'] += $il['qty'];
					$item_list_total['cost_total'] += $il['cost'];
					$item_list_total['total_total'] += $il['total'];
					$il = array();
				}
			...
			if($sessioninfo['u']=='admin'){
			//	print ("b_count = $b_count , after get item list: ".memory_get_usage()."<br>==========<br>");
			}
			
			$sid_count = count($sid_list);
			for($i=0; $i<$sid_count; $i+=5000){
				...
				$sku_item_have_closing_cost = array();
				while($r = $con_multi->sql_fetchassoc($q_cc)){
	
				    $sid = $r['sid'];
		            $sku[$sid]['closing_cost'] = $r['grn_cost'];
				}
	...
	*/
			...
			if (grn_items.acc_cost is null, grn_items.cost,grn_items.acc_cost)) as total_rcv_cost $tmp_col
			from grn_items
			...
			group by sid";
				...
				$con_multi->sql_query($sql) or die(mysql_error());
			 	while($r = $con_multi->sql_fetchrow()){
					...
					$table[$key]['do'] += $r['qty'];
					$table[$key]['do_cost'] += $r['qty'] * $sku[$sid]['closing_cost'];
				}
				$con_multi->sql_freeresult();
				if($sessioninfo['u']=='admin'){
		...
		//					print $sc_date."    ".$sid."  =  ".$r['cost']."  <br />";
							...
							if($opening_sc_balance_val[$sc_date][$sid]['cost']){
								...
								$new_cost = $sku[$sid]['closing_cost']*$opening_sc_balance_val[$sc_date][$sid]['qty'];
							}
	...
	//				print $sid."  =  ".$r['cost']."  <br />";
				...
				if($sc_balance_sids){
					...
					foreach ($sc_balance_sids as $sc_date => $sids){
						$minus_1_day=strtotime("-1 day",strtotime($sc_date));
						$sb_year=date("Y",$minus_1_day);
						$sc1day_date=ms(date("Y-m-d",$minus_1_day));
...
group by sid";
					...
					while($r = $con_multi->sql_fetchrow($q_cn)){
						if(!$r['dont_count']){
							...
							if(!$r['bal']){
								...
								$table[$key]['cn_val']+=$qty * $sku[$sid]['closing_cost'];
							}
						}
					}
					...
					while($r = $con_multi->sql_fetchrow($q_dn)){
						if(!$r['dont_count']){
							...
							if(!$r['bal']){
								...
								$table[$key]['dn_bal']+=$qty * $sku[$sid]['closing_cost'];
							}
						}
					}
		...
		*/
        ...
        $smarty->assign('item_list_total',$item_list_total);
        ...
        if($_REQUEST['ajax']){
			...
			$this->display('hasani/report.stock_balance_summary.row.tpl');
		}else{
            $this->display("hasani/report.stock_balance_summary.tpl");
		}
	}
