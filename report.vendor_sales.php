<?php
/*
2/20/2013 4:06 PM Andy
- A new report going to replace old "Daily Vendor Sales Report".

8/27/2013 2:04 PM Justin
- Enhanced to follow sequence by price type, sku item code and mcode.

12/31/2013 3:00 PM Andy
- Fix to get finalized sales only.

11/27/2014 5:31 PM Andy
...
6/16/2015 3:27 PM Justin
- Bug fixed on artno/mcode will show empty if it is equal to "".
...
10/23/2018 5:42 PM Justin
- Bug fixed on Vendor filter no longer working.

2/20/2020 2:00 PM William
- Enhanced to change connection "$con" to use report server connection "$con_multi".

10/13/2020 3:54 PM William
...
class VENDOR_SALES_REPORT extends Module {
	...
	private function init_load(){
		...
		// vendor
		$con_multi->sql_query("select id,code,description from vendor where active=1 order by description");
		while($r = $con_multi->sql_fetchassoc()){
	...
	private function generate_report(){
		...
		if(BRANCH_CODE == 'HQ'){
			...
			$b_info = $this->branch_list[$form['branch_id']];
		}else{
			// branch
			$bid_list = array($sessioninfo['branch_id']);
			$report_title[] = "Branch: ".BRANCH_CODE;
			$b_info = $this->branch_list[$sessioninfo['branch_id']];
		}
		
		// validate parameters
		$this->vendor_id = mi($form['vendor_id']);
		...
		$this->use_grn = mi($form['use_grn']);
		//$this->show_all_items = mi($form['show_all_items']);
		$this->group_by_sku = mi($form['group_by_sku']);
		$this->show_balance = mi($form['show_balance']);
		
		// report type
		$this->report_type = trim($form['report_type']);
		
		$err = array();
		
		if(!$bid_list)	$err[] = "Please select branch.";
		if(!$this->vendor_id)	$err[] = "Please select vendor.";
		...
		//if($this->show_all_items && !$this->dept_id)	$err[] = "Show All Items cannot use for All Department. Please select single Department.";
		if($err){
			$smarty->assign('err', $err);
			return false;
		}
		$report_title[] = "Vendor: ".$this->vendor_list[$this->vendor_id]['description'];
		...
		switch($form['report_type']){
			case 'amt':
				$report_type = 'Sales Amt';	break;
			case 'qty':
				$report_type = 'Sales Qty';	break;
			case 'gst':
				$report_type = 'Tax Amt';	break;
			default:
				$report_type = 'Amt Inc Tax';
		}
		$report_title[] = "Report Type: $report_type";
		if($this->use_grn)	$report_title[] = "Use GRN: YES";
		...
		$this->data = $this->is_under_gst = array();
		...
		if($this->data['item_sales']){
			uksort($this->data['item_sales'], array($this, "sort_item_id"));
		}
		//print_r($this->data);
	...
	private function generate_data_by_branch($bid){
		...
		// filter
		$filter = array();
		$filter_use_grn = array();
		
		$filter[] = "pos.branch_id=".mi($bid);
		
		if($this->use_grn){	// use grn
			$filter_use_grn[] = "vsh.vendor_id=".$this->vendor_id;
		}else{	// no use grn, use master vendor
		...
		if($filter_use_grn)	$sales_filter .= ' and '.join(' and ', $filter_use_grn);
		
		
		// xtra join
		$xtra_join = '';
		if($this->use_grn){
			$xtra_join .= "join vendor_sku_history_b".$bid." vsh on vsh.sku_item_id=pi.sku_item_id and pi.date between vsh.from_date and vsh.to_date and vsh.vendor_id=".mi($this->vendor_id);
		}
		...
		foreach($this->date_from_to_list as $tmp_key => $tmp_date_from_to_info){
			...
			$sql = "select sum(pi.price-pi.discount-pi.discount2-pi.tax_amount) as amt, sum(pi.qty) as qty, sum(pi.tax_amount) as gst_amt, sku.sku_type, month(pos.date) as month, year(pos.date) as year,
					pos.date as dt, si.sku_item_code, if(si.artno is null or si.artno = '',si.mcode, si.artno) as artno_mcode, si.description,
					pi.trade_discount_code as price_type,pi.sku_item_id,si.sku_id,uom.fraction as packing_uom_fraction,si.is_parent, pos.is_gst, pos.is_tax_registered
					from pos
					...
					$xtra_join $sales_filter
					group by dt, pi.sku_item_id, price_type 
					order by price_type, si.sku_item_code, si.mcode";
			//print $sql."<br>";
			...
			while($r = $con_multi->sql_fetchassoc($q1)){
				...
				if(($r['is_gst'] || $r['is_tax_registered']) && !isset($this->is_under_gst[$ym])) $this->is_under_gst[$ym] = true;
				...
				$this->data['item_sales'][$key][$ym][$price_type][$r['dt']]['gst'] += $gst_amt;
				$this->data['item_sales'][$key][$ym][$price_type][$r['dt']]['amt_inc_gst'] += $amt + $gst_amt;
				...
				$this->data['item_sales'][$key][$ym][$price_type]['total']['gst'] += $gst_amt;
				$this->data['item_sales'][$key][$ym][$price_type]['total']['amt_inc_gst'] += $amt + $gst_amt;
				
				// item sales total by row
				$this->data['item_total'][$key][$ym]['qty'] += $qty;
				$this->data['item_total'][$key][$ym]['amt'] += $amt;
				$this->data['item_total'][$key][$ym]['gst'] += $gst_amt;
				$this->data['item_total'][$key][$ym]['amt_inc_gst'] += $amt + $gst_amt;
				...
				if($this->by_monthly){
					...
					$this->data['item_total'][$key]['by_year'][$tmp_key]['gst'] += $gst_amt;
					$this->data['item_total'][$key]['by_year'][$tmp_key]['amt_inc_gst'] += $amt + $gst_amt;
				...
				$this->date_from_to_list[$tmp_key]['group_total']['sales'][$group_total_key]['gst'] += $gst_amt;
				$this->date_from_to_list[$tmp_key]['group_total']['sales'][$group_total_key]['amt_inc_gst'] += $amt + $gst_amt;
				...
				$this->date_from_to_list[$tmp_key]['group_total']['sales']['total']['gst'] += $gst_amt;
				$this->date_from_to_list[$tmp_key]['group_total']['sales']['total']['amt_inc_gst'] += $amt + $gst_amt;
			...
			// group by sku, get the sales by other parent/child
			if($this->group_by_sku && $this->use_grn && $this->date_from_to_list[$tmp_key]['sku_id_list']){
				$sku_id_list = array();
				foreach($this->date_from_to_list[$tmp_key]['sku_id_list'] as $sku_id => $tmp_r){
					if($tmp_r[$bid]){	// this branch got sales
						$sku_id_list[] = $sku_id;
					}
				}
				
				if($sku_id_list){
					$grp_sku_filter = array();
					$grp_sku_filter[] = "si.sku_id in (".join(',', $sku_id_list).")";
					
					$tmp_filter = $sales_filter." and ".join(' and ', $grp_sku_filter);
					
					$sql = "select sum(pi.price-pi.discount) as amt, sum(pi.qty) as qty, sum(pi.tax_amount) as gst_amt, sku.sku_type, month(pos.date) as month, year(pos.date) as year,
							pos.date as dt, si.sku_item_code, if(si.artno is null or si.artno = '',si.mcode, si.artno) as artno_mcode, si.description,
							pi.trade_discount_code as price_type,pi.sku_item_id,si.sku_id,uom.fraction as packing_uom_fraction,si.is_parent, pos.is_gst, pos.is_tax_registered
							from pos
							left join pos_items pi on pi.branch_id=pos.branch_id and pi.pos_id=pos.id and pi.date=pos.date and pi.counter_id=pos.counter_id
					...
					while($r = $con_multi->sql_fetchassoc($q2)){
						...
						// check this date can add sales or not
						if(!isset($this->data['item_sales'][$key][$ym]))	continue;
						else{
							$can_add = false;
							// loop for each price type, check this date got sales or not
							foreach($this->data['item_sales'][$key][$ym] as $tmp_price_type => $tmp_date_list){
								if(isset($tmp_date_list[$r['dt']])){	// got sales
									$can_add = true;
									break;
								}
							}
						...
						// item sales, only add if the sales not yet get from the 1st query
						if(!$this->data['item_sales'][$key][$ym][$price_type][$r['dt']]['sid_list'][$sid][$bid]){
							// item sales
							$this->data['item_sales'][$key][$ym][$price_type][$r['dt']]['qty'] += $qty;
							$this->data['item_sales'][$key][$ym][$price_type][$r['dt']]['amt'] += $amt;
							$this->data['item_sales'][$key][$ym][$price_type][$r['dt']]['gst'] += $gst_amt;
			...
			if($this->group_by_sku && $this->data){	
				...
				if($sku_id_list_to_get){
					$con_multi->sql_query("select si.id as sku_item_id, si.sku_item_code, if(si.artno is null or si.artno = '',si.mcode, si.artno) as artno_mcode, si.description, sku.sku_type, si.sku_id
					from sku_items si
	...
	private function construct_item_info($r){
		$info_fields = array('sku_item_id', 'sku_item_code', 'artno_mcode', 'description', 'sku_type', 'sku_id');
...
$VENDOR_SALES_REPORT = new VENDOR_SALES_REPORT('Daily Vendor Sales Report');
?>
