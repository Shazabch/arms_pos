...
<nav aria-label="breadcrumb m-0 mb-2">
	<ol class="breadcrumb bg-white animated fadeInDown">
		...
		<li class="breadcrumb-item">
			<a href="{$smarty.server.PHP_SELF}?a=open&do_no={$search_var}">Back to search</a>
		</li>
	</ol>
</nav>
<!-- /BreadCrumbs -->
...
<div class="row mt-2">
					...
					<div class="pd-10 pd-sm-20">
						<div class="row row-xs">
							<div class="col-md-1 text-center">
								<label><b>Scan</b></label>
							</div>
							<div class="col-md-5 mg-t-10 mg-md-t-0">
								<input class="form-control" type="text" name="product_code">
								{if $config.enable_grn_barcoder}
								<label class="rdiobox mt-2"><input type="radio" name="grn_barcode_type" value="0" {if $use_barcode_type eq 0}checked{/if}> <span>GRN Barcoder</span></label>
								{/if}
								<label class="rdiobox mt-2"><input type="radio" name="grn_barcode_type" value="1" {if $use_barcode_type eq 1}checked{/if}> <span class="fs-08">ARMS Code / MCode / Art.No / {$config.link_code_name}</span></label>
							</div>
	...
	<div class="col-lg-12 col-md-12">
		{if $result}
			{if $result.not_found}
				<div class="alert alert-danger animated fadeInDown">NOT FOUND</div>
			{elseif $result.data}
				<div class="alert alert-success animated fadeInDown text-center"><b>FOUND</b></div>
