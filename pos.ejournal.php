...
2/26/2020 11:38 AM William
- Enhanced to change connection "$con" to use report server connection "$con_multi".
*/
include("include/common.php");
if (!$login) js_redirect($LANG['YOU_HAVE_LOGGED_OUT'], "/index.php");
if (!privilege('FRONTEND_EJOURNAL')) js_redirect(sprintf($LANG['NO_PRIVILEGE'], 'FRONTEND_EJOURNAL', BRANCH_CODE), "/index.php");

class POS_EJOURNAL extends Module{
	var $file_folder = "tmp/ejournal";
	...
	function init(){
	 ...
	 function load_data(){
		  ...
		  $mode = '';
		  if($form['counter_id']){
			   $filter[] = "pte.branch_id = ".$form['branch_id'];
			   $filter[] = "pte.counter_id = ".$form['counter_id'];
			   if($form['date']) {
					$filter[] = "pte.date = ".ms($form['date']);
					$mode = 'date';
			   }else{
					$filter[] = "pte.date between ".ms($form['date_from'])." and ".ms($form['date_to']);
					$mode = 'counter';
			   }
		  }else{
			   ...
			   if($form['counters']) {
					...
					$filter[] = "pte.branch_id = $branch_id";
					if($counter_id != 'all')    $filter[] = "pte.counter_id = $counter_id";
					$mode = 'all';
			   }
		  ...
		  $con_multi->sql_query("select pte.*, b.code, cs.network_name
						  from pos_transaction_ejournal pte
						  left join branch b on b.id=pte.branch_id
						  left join counter_settings cs on cs.id=pte.counter_id and cs.branch_id=pte.branch_id
						  where $filter order by pte.branch_id, pte.counter_id, pte.date");
		  ...
		  while($r = $con_multi->sql_fetchassoc()){
			   if($form['type'] == 'show') {
					...
					$item_list[$r['branch_id']."_".$r['counter_id']]['info'][$r['date']] = $r['ejournal_log'];
		  ...
		  if($form['type'] == 'show') {
			   ...
			   if($mode == 'date') {
					...
					readfile($output);
					log_br($sessioninfo['id'], 'REPORT_EXPORT', 0, "Export E-Journal to TXT Format");
			   }else {
					$parent_zip = "EJOURNAL_".$times;
					...
					log_br($sessioninfo['id'], 'REPORT_EXPORT', 0, "Export E-Journal to ZIP File Format");
			   }
		  }
	 }
}

$POS_EJOURNAL = new POS_EJOURNAL("E-Journal");
?>