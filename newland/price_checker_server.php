...
function get_item($msgsock, $msg){
    ...
    if ($sku['error'])
	{
		...
		if(!$member){
			...
			$output .= tohex('1b').tohex('24');	// clear screen
			$output .= $msg.tohex('0d');
			
			$output .= tohex('1b').tohex('42').tohex('31');
			$output .= tohex('1b').tohex('2e').tohex('36')."Item Not Found".tohex('03');
			$output .= tohex('1b').tohex('2e').tohex('38').tohex('03');
		}else{ // member info
			$output = tohex('1b')."%";
			$output .= tohex('1b').tohex('42').tohex('30');
			$output .= tohex('1b').tohex('24');
			$output .= $member['name'].tohex('0d').tohex('0d').tohex('0d')."IC: ".$member['nric'].tohex('0d')."Points: ".$member['points'].tohex('0d')."Points Update: ".$member['points_update'].tohex('0d');
			
			// bottom left
			$output .= tohex('1b').tohex('2e').tohex('36')."Expiry Date: ".$member['next_expiry_date'].tohex('03');
		}
	}else{
        $output = tohex('1b')."%";
		$output .= tohex('1b').tohex('42').tohex('30');
		...
		//$output .= $msg;
		$desc = $sku['receipt_description'];
		if(strlen($desc)>20){
			for($i = 0, $len = strlen($desc), $span=20; $i < $len && $i < $span*2; $i+=$span){
				$output .= substr($desc, $i, $span).tohex('0d');		
			}
		}else	$output .= $desc.tohex('0d');
		...
		if($sku['member_price']>0 && $sku['member_price'] != $sku['price']){
            ...
            if($sku['member_price']==$sku['non_member_price']){
			    ...
			    if(!$config['shuttle_sg_only_show_nett_price']){
       			...
       			$output .= tohex('1b').tohex('42').tohex('31');
		    	//$output .= tohex('1b').tohex('2c').tohex('33').tohex('50')."Nett";
		    	$output .= tohex('1b').tohex('2e').tohex('33')."Nett".tohex('03');
		    	$output .= tohex('1b').tohex('2e').tohex('35').$config["arms_currency"]["symbol"].number_format($price,2).$str_gst_indicator.tohex('03');
