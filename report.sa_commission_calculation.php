...
1/27/2015 4:28 PM Justin
- Bug fixed on cost have been calculated wrongly.
   ...
   function __construct($title){
	...
	private function run_report($bid){
		...
		while($r1 = $con_multi->sql_fetchrow($q1)){
			...
			if($r1['type'] == "DO"){
				// calculation for cost price by markup
				/*$cost_price = $r1['cost_price'];
				if($r1['do_markup']){
					$temp = array();
					$temp['do_markup_arr'] = explode("+", $r1['do_markup']);
					if($r1['markup_type']=='down'){
						$temp['do_markup_arr'][0] *= -1;
						$temp['do_markup_arr'][1] *= -1;
					}
					if($temp['do_markup_arr'][0]){
						$cost_price = $cost_price * (1+($temp['do_markup_arr'][0]/100));
					}
					if($temp['do_markup_arr'][1]){
						$cost_price = $cost_price * (1+($temp['do_markup_arr'][1]/100));
					}
				}
				$row_amt = round(($cost_price/$r1['fraction'])*$r1['qty'], 2);
				
				// found having item discount
				if($r1['item_discount']){
					$temp = array();
					$row_discount_amt = 0;
					$temp['item_discount_arr'] = explode("+", $r1['item_discount']);
					if($temp['item_discount_arr'][0]){
						if(preg_match("/%/", $temp['item_discount_arr'][0]))
							$row_amt = $row_amt - ($row_amt*($temp['item_discount_arr'][0]/100));
						else
							$row_amt -= $temp['item_discount_arr'][0];
					}
					if($temp['item_discount_arr'][1]){
						if(preg_match("/%/", $temp['item_discount_arr'][1]))
							$row_amt = $row_amt - ($row_amt*($temp['item_discount_arr'][1]/100));
						else
							$row_amt -= $temp['item_discount_arr'][1];
					}
				}*/
