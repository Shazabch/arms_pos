...
6/12/2019 1:08 PM William
- Added new moq by csv.
- Added new checking to disable Moq value larger than Max value.
- Added new checking to disable Min value equal or large than Max value.
*/
define('TERMINAL', 1);
...
function import_sku($filename, $real_import = false){
	...
	$allowed_sku_type = array("CONSIGNMENT"=>'CONSIGN',"CONSIGN"=>'CONSIGN',"OUTRIGHT"=>'OUTRIGHT',"CONCESS"=>'CONCESS',"CONCESSION"=>'CONCESS',"CONCESSIONAIRE"=>'CONCESS');
	$infile_parent_data = array();
	$default_line_id = $default_vendor_id = 0;
	$default_dept_id = $default_cat1_id = array();
			
	$error_header = array('mcode','link_code','artno','description','receipt_description','uom','cost_price','selling_price','scale_type','wholesale1','line','department','category1','category2','category3','category4','category5','brand','vendor','color','size','input_tax','output_tax','sku_type','po_reorder_qty_min','po_reorder_qty_max','po_reorder_moq','parent_arms_code','parent_mcode','parent_artno','trade_discount_type','trade_discount_code','trade_discount_rate');
	...
	while($r = fgetcsv($f)){
		...
		$ins['parent_mcode'] = preg_replace("/[^A-Za-z0-9]/", "", trim($r[28]));
		...
		//check moq and min qty
		if(!$ins['po_reorder_qty_min'] && !$ins['po_reorder_qty_max']  && $ins['po_reorder_moq']){
			$ins['error'] = "PO reorder qty max and min must fill if exist MOQ";
		}elseif($ins['po_reorder_qty_min'] && $ins['po_reorder_qty_max']  || $ins['po_reorder_moq']){
			if($ins['po_reorder_qty_min'] >= $ins['po_reorder_qty_max']){
				$ins['error'] = "PO reorder qty max cannot less than or equal min";
			}
			if($ins['po_reorder_moq'] > $ins['po_reorder_qty_max']){
				$ins['error'] = "PO reorder qty max cannot less than MOQ";
			}
		}
		if($ins['po_reorder_qty_min'] || $ins['po_reorder_qty_max']){
			if($ins['po_reorder_qty_min'] >= $ins['po_reorder_qty_max']){
