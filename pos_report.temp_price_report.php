...
- Bug fixed on incorrect database query syntax
*}
<?php
...
class TEMP_PRICE_HISTORY extends Module{
	...
	function _default()
	{
		...
		$smarty->display('pos_report.temp_price_report.tpl');
	}
	...
	private function load_report()
	{
		...
		if (BRANCH_CODE!='HQ') 
		{
			$branch = " and lph.branch_id =".mi($sessioninfo[branch_id]);
		...
		{
			$branch = " and lph.branch_id=".mi($branch_id);
			$con->sql_query("select * from branch Where id=".mi($branch_id));
			$br = $con->sql_fetchrow();
			$report_header['branch'] = $br['code'];
			$con->sql_freeresult();
...
$TEMP_PRICE_HISTORY = new TEMP_PRICE_HISTORY('Temp Price Report');
?>