...
function run($branch)
{
	...
	//$con->sql_query("drop table if exists pwp_sales_cache_b$bid");
    $con->sql_query("
	create table if not exists pwp_sales_cache_b$bid (
	date date,
	year int(4),
	month int(2),
	day int(2),
	...
	$con->sql_query("alter table pwp_sales_cache_b$bid convert to charset latin1 collate latin1_general_ci");
	$con->sql_query("alter table pwp_sales_cache_b$bid add qty double after transaction_count",false,false);
	...
	else
	{
		...
		$con->sql_query("select max(date) from pwp_sales_cache_b$bid");
	...
	if($ddel!=''){
        $con->sql_query("delete from pwp_sales_cache_b$bid where $ddel");
		print $con->sql_affectedrows()." rows deleted.";
	}
	
	
	$rs = $con->sql_query("select date(timestamp) as date,year,month,day,count(distinct(transaction_id)) as transaction_count,sum(qty) as qty, sum(amount) as amount,sum(pos.grn_cost*abs(pos.qty)) as cost,sku_item_code from pos_transaction pos where branch_id = $bid $date_select and pwp=".ms("PWP")." group by year,month,day,sku_item_code");
    ...
    while($r=$con->sql_fetchrow($rs))
	{
		$values = xr($r, $fields);
		if ($n==0)
			$query = "insert into pwp_sales_cache_b$bid (".join(",", $fields).") values (".join(",",$values).")";
		else
			$query .= ", (".join(",",$values).")";
		$n++;
	...
	print " saved.\n";
	$con->sql_query("optimize table pwp_sales_cache_b$bid");
	$con->sql_query("analyze table pwp_sales_cache_b$bid");
}
...
where bgi.branch_group_id=".mi($bg_id);
	...
	if(!$allbranch){
 	...
 	$con->sql_query("
	create table if not exists pwp_sales_cache_bg$bg_id (
	date date,
	year int(4),
	month int(2),
	day int(2),
	...
	index(year),index(month),index(day))
	");
	$con->sql_query("alter table pwp_sales_cache_bg$bg_id convert to charset latin1 collate latin1_general_ci");
	$con->sql_query("alter table pwp_sales_cache_bg$bg_id add qty double after transaction_count",false,false);
	
	// date checking
	if ($datefilter == 'last')
	{
		$con->sql_query("select max(date) from pwp_sales_cache_bg$bg_id");
		$startd = $con->sql_fetchrow();
	...
	if($where){
		// delete data
		$con->sql_query("delete from pwp_sales_cache_bg$bg_id $where") or die(mysql_error());
	}
	
	foreach($branches_list as $bid){
		$target_tbl = "pwp_sales_cache_bg$bg_id";
		$from_tbl = "pwp_sales_cache_b$bid";

		$sql = "insert into $target_tbl (select * from $from_tbl $where) ON DUPLICATE KEY UPDATE
		$target_tbl.qty=$target_tbl.qty+$from_tbl.qty,
		$target_tbl.amount=$target_tbl.amount+$from_tbl.amount,
		$target_tbl.cost=$target_tbl.cost+$from_tbl.cost,
