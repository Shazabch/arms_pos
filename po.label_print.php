<?php
/*
6/23/2009 5:04 PM Andy
- add cheking on $config['po_label_alt_print_template'] to allow custom print
...
*/
	...
	$fp = fopen($send_to, "w");
	if($config['po_label_alt_print_template'])  fwrite($fp, $smarty->fetch($config['po_label_alt_print_template']));
	else	fwrite($fp, $smarty->fetch("po.label_print.datamax.tpl"));
	fclose($fp);
